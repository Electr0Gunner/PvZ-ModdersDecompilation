...
ChallengeScreen::ChallengeScreen(LawnApp* theApp, ChallengePage thePage)
{
	...
	mBackButton = MakeNewButton(ChallengeScreen::ChallengeScreen_Back, this, "[BACK_TO_MENU]", nullptr, PopLib::IMAGE_SEEDCHOOSER_BUTTON2, 
		PopLib::IMAGE_SEEDCHOOSER_BUTTON2_GLOW, PopLib::IMAGE_SEEDCHOOSER_BUTTON2_GLOW);
	...
	for (int aPageIdx = CHALLENGE_PAGE_SURVIVAL; aPageIdx < MAX_CHALLANGE_PAGES; aPageIdx++)
	{
		...
		aPageButton->mButtonImage = PopLib::IMAGE_BLANK;
		aPageButton->mOverImage = PopLib::IMAGE_BLANK;
		aPageButton->mDownImage = PopLib::IMAGE_BLANK;
		aPageButton->SetFont(PopLib::FONT_BRIANNETOD12);
...
void ChallengeScreen::DrawButton(Graphics* g, int theChallengeIndex)
{
	...
	if (aChallengeButton->mVisible)
	{
		...
		if (AccomplishmentsNeeded(theChallengeIndex) <= 1)
		{
			...
			{
				g->DrawImageCel(PopLib::IMAGE_SURVIVAL_THUMBNAILS, aPosX + 13, aPosY + 4, aDef.mChallengeIconIndex);
			}
			else
			{
				g->DrawImageCel(PopLib::IMAGE_CHALLENGE_THUMBNAILS, aPosX + 13, aPosY + 4, aDef.mChallengeIconIndex);
			}
			...
			g->DrawImage(aHighLight ? PopLib::IMAGE_CHALLENGE_WINDOW : PopLib::IMAGE_CHALLENGE_WINDOW_HIGHLIGHT, aPosX - 6, aPosY - 2);

			// ============================================================================================
			// ▲ 绘制小游戏的名称
			// ============================================================================================
			Color aTextColor = aHighLight ? Color(250, 40, 40) : Color(42, 42, 90);
			PopString aName = TodStringTranslate(aDef.mChallengeName);
			...
			if (aNameLen < 13)
			{
				TodDrawString(g, aName, aPosX + 52, aPosY + 96, PopLib::FONT_BRIANNETOD12, aTextColor, DS_ALIGN_CENTER);
			}
			else
			{
				...
				const PopChar* aSpacedChar = strchr(aName.c_str() + aHalfPos, ' ');
				while(aSpacedChar[0]!=' ')
				{
					aHalfPos++;
				...
				{
					TodDrawString(g, topStr, aPosX + 52, aPosY + 88, PopLib::FONT_BRIANNETOD12, aTextColor, DS_ALIGN_CENTER);
					if (aLine2Len > 0)
					{
						TodDrawString(g, botStr, aPosX + 52, aPosY + 102, PopLib::FONT_BRIANNETOD12, aTextColor, DS_ALIGN_CENTER);
					}
				}
			...
			if (theChallengeIndex == mUnlockChallengeIndex)
			{
				Image* aLockImage = PopLib::IMAGE_LOCK;
				if (mUnlockState == UNLOCK_FADING)
				{
					aLockImage = PopLib::IMAGE_LOCK_OPEN;
			...
			else if (aRecord > 0)
			{
				...
				{
					g->DrawImage(PopLib::IMAGE_MINIGAME_TROPHY, aPosX - 6, aPosY - 2);
				}
				else if (mApp->IsEndlessScaryPotter(aDef.mChallengeMode) || mApp->IsEndlessIZombie(aDef.mChallengeMode))
				{
					PopString aAchievement = mApp->Pluralize(aRecord, "[ONE_FLAG]", "[COUNT_FLAGS]");
					TodDrawString(g, aAchievement, aPosX + 48, aPosY + 48, PopLib::FONT_CONTINUUMBOLD14OUTLINE, Color::White, DS_ALIGN_CENTER);
					TodDrawString(g, aAchievement, aPosX + 48, aPosY + 48, PopLib::FONT_CONTINUUMBOLD14, Color(255, 0, 0), DS_ALIGN_CENTER);
				}
				else if (mApp->IsSurvivalEndless(aDef.mChallengeMode))
				{
					PopString aAchievement = TodReplaceNumberString("[LONGEST_STREAK]", "{STREAK}", aRecord);
					Rect aRect(aPosX, aPosY + 15, 96, 200);
					TodDrawStringWrapped(g, aAchievement, aRect, PopLib::FONT_CONTINUUMBOLD14OUTLINE, Color::White, DS_ALIGN_CENTER);
					TodDrawStringWrapped(g, aAchievement, aRect, PopLib::FONT_CONTINUUMBOLD14, Color(255, 0, 0), DS_ALIGN_CENTER);
				}
			}
			else if (aChallengeButton->mDisabled)
			{
				TodDrawImageScaledF(g, PopLib::IMAGE_LOCK, aPosX + 24, aPosY + 9, 0.7f, 0.7f);
			}
		}
		else
		{
			g->DrawImage(PopLib::IMAGE_CHALLENGE_BLANK, aPosX, aPosY);
		}
	}
}
...
void ChallengeScreen::Draw(Graphics* g)
{
	...
	g->DrawImage(PopLib::IMAGE_CHALLENGE_BACKGROUND, 0, 0);

	PopString aTitleString = 
		mPageIndex == CHALLENGE_PAGE_SURVIVAL ? "[PICK_AREA]" : 
		mPageIndex == CHALLENGE_PAGE_PUZZLE ? "[SCARY_POTTER]" : "[PICK_CHALLENGE]";
	TodDrawString(g, aTitleString, 400, 58, PopLib::FONT_HOUSEOFTERROR28, Color(220, 220, 220), DS_ALIGN_CENTER);
	...
	if (aTrophiesTotal > 0)
	{
		PopString aTrophyString = StrFormat("%d/%d", aTrophiesGot, aTrophiesTotal);
		TodDrawString(g, aTrophyString, 739, 73, PopLib::FONT_DWARVENTODCRAFT12, Color(255, 240, 0), DS_ALIGN_CENTER);
	}
	TodDrawImageScaledF(g, PopLib::IMAGE_TROPHY, 718, 26, 0.5f, 0.5f);
