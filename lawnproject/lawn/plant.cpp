...
void Plant::PlantInitialize(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType)
{
    ...
    if (aPlantDef.mReanimationType != ReanimationType::REANIM_NONE)
    {
        ...
        mBlinkCountdown = 400 + PopLib::Rand(400);
    }
    ...
    case SeedType::SEED_WALLNUT:
        mPlantHealth = 4000;
        mBlinkCountdown = 1000 + PopLib::Rand(1000);
        break;
    ...
    case SeedType::SEED_PLANTERN:
    {
        ...
        if (IsInPlay())
        {
            mApp->PlaySample(PopLib::SOUND_PLANTERN);
        }
...
void Plant::UpdateShooter()
{
    ...
    if (mLaunchCounter <= 0)
    {
        mLaunchCounter = mLaunchRate - PopLib::Rand(15);
...
void Plant::UpdateProductionPlant()
{
    ...
    if (mLaunchCounter <= 0)
    {
        ...
        {
            mBoard->AddCoin(mX, mY, (PopLib::Rand(100) < 10) ? CoinType::COIN_GOLD : CoinType::COIN_SILVER, CoinMotion::COIN_MOTION_COIN);
        }
...
void Plant::UpdatePotato()
{
    ...
    else if (mState == PlantState::STATE_POTATO_RISING)
    {
        if (aBodyReanim->mLoopCount > 0)
        {
            ...
            mState = PlantState::STATE_POTATO_ARMED;
            mBlinkCountdown = 400 + PopLib::Rand(4000);
        }
    }
...
bool Plant::DrawMagnetItemsOnTop()
{
    ...
    if (mSeedType == SeedType::SEED_MAGNETSHROOM)
    {
        for (int i = 0; i < MAX_MAGNET_ITEMS; i++)
        {
            ...
            {
                Vector2 aVectorToPlant(mX + aMagnetItem->mDestOffsetX - aMagnetItem->mPosX, mY + aMagnetItem->mDestOffsetY - aMagnetItem->mPosY);
...
void Plant::UpdateGoldMagnetShroom()
{
    ...
    else if (!IsAGoldMagnetAboutToSuck() && PopLib::Rand(50) == 0 && FindGoldMagnetTarget())
    {
        mBoard->ShowCoinBank();
        mState = PlantState::STATE_MAGNETSHROOM_SUCKING;
...
void Plant::UpdateBowling()
{
    ...
    if (aZombie)
    {
        ...
        if (mSeedType != SeedType::SEED_GIANT_WALLNUT)
        {
            ...
            {
                aNewState = PopLib::Rand(2) ? PlantState::STATE_BOWLING_UP : PlantState::STATE_BOWLING_DOWN;
            }
        }
    }
...
//0x465680
void Plant::DrawShadow(PopLib::Graphics* g, float theOffsetX, float theOffsetY)
{
...
void Plant::DrawSeedType(Graphics* g, SeedType theSeedType, SeedType theImitaterType, DrawVariation theDrawVariation, float thePosX, float thePosY)
{
    ...
    if (((LawnApp*)gAppBase)->mGameMode == GameMode::GAMEMODE_CHALLENGE_BIG_TIME &&
...
//0x467C00
PopString Plant::GetNameString(SeedType theSeedType, SeedType theImitaterType)
{
    const PlantDefinition& aPlantDef = GetPlantDefinition(theSeedType);
    PopString aName = StrFormat("[%s]", aPlantDef.mPlantName);
    PopString aTranslatedName = TodStringTranslate(aName);
...
//0x467DB0
PopString Plant::GetToolTip(SeedType theSeedType)
{
    const PlantDefinition& aPlantDef = GetPlantDefinition(theSeedType);
    PopString aToolTip = StrFormat("[%s_TOOLTIP]", aPlantDef.mPlantName);
