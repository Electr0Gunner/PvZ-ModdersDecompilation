...
void CutScene::PreloadResources()
{
	...
	mBoard->mPreloadTime = std::max((int)aTimer.GetDuration(), 0);
...
void CutScene::UpdateZombiesWon()
{
	...
	if (mCutsceneTime == LostTimeEnd)
	{
		if (mApp->IsSurvivalMode())
		{
			...
			PopString aFlagsStr = mApp->Pluralize(aFlagsCompleted, "[ONE_FLAG]", "[COUNT_FLAGS]");
			PopString aStr = TodReplaceString("[SURVIVAL_DEATH_MESSAGE]", "{FLAGS}", aFlagsStr);
...
void CutScene::AdvanceCrazyDaveDialog(bool theJustSkipping)
{
	...
	if ((aMessageIndex == 1503 || aMessageIndex == 1553) && !theJustSkipping)
	{
		...
		PopString aBodyString = TodReplaceNumberString("[UPGRADE_DIALOG_BODY]", "{SLOTS}", aNumPackets + 1);
		PopString aAmountString = mApp->GetMoneyString(mApp->mPlayerInfo->mCoins);
		// 创建询问是否升级卡槽格数的对话
...
{
	PopString aCrazyDaveText = mApp->GetCrazyDaveText(mCrazyDaveLastTalkIndex);
	int anIndex = aCrazyDaveText.find("{DELAY_");
	if (anIndex != PopString::npos)
	{
		PopString aDelayTimeStr = aCrazyDaveText.substr(anIndex + 7, aCrazyDaveText.find("}") - anIndex - 7);
...
int CutScene::ParseTalkTimeFromMessage()
{
	...
	if (anIndex != -1)
	{
		PopString aTalkTimeStr = aCrazyDaveText.substr(anIndex + 6, aCrazyDaveText.find("}") - anIndex - 6);
