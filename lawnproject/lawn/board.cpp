...
#include "lawn/system/savegame.hpp"
#include "lawn/widget/lawndialog.hpp"
...
#include <todlib/effects/trail.hpp>
...
{
	Rect aRect(GetSeedBankExtraWidth() + 456, 0, PopLib::IMAGE_SHOVELBANK->GetWidth(), PopLib::IMAGE_SEEDBANK->GetHeight());
...
void Board::GetZenButtonRect(GameObjectType theObjectType, Rect& theRect)
{
	...
	for (int anObject = GameObjectType::OBJECT_TYPE_WATERING_CAN; anObject < theObjectType; anObject++)
	{
		...
		if (CanUseGameObject((GameObjectType)anObject))
		{
			theRect.mX += 70;//PopLib::IMAGE_SHOVELBANK->GetWidth();
		}
	}
	//return theRect;
}
...
void Board::UpdateLevelEndSequence()
{
	...
	if (mBoardFadeOutCounter == 300)
	{
		...
		{
			mApp->PlaySample(PopLib::SOUND_LIGHTFILL);
		}
	}
	...
	if (CanDropLoot() && !IsSurvivalStageWithRepick())
	{
		...
		if (aLawnMower)
		{
			...
			SoundInstance* aSoundInstance = mApp->mSoundManager->GetSoundInstance(PopLib::SOUND_POINTS);
			if (aSoundInstance)
			{
...
void Board::FadeOutLevel()
{
	...
	if (mApp->IsScaryPotterLevel() && !IsFinalScaryPotterStage())
	{
		...
		else
		{
			...
			PopString aStreakStr = mApp->IsEndlessScaryPotter(mApp->mGameMode) ? "[ADVICE_MORE_SCARY_POTS]" : "[ADVICE_3_IN_A_ROW]";
			PopString aMessage = TodReplaceNumberString(aStreakStr, "{STREAK}", mChallenge->mSurvivalStage + 1);
			PuzzleSaveStreak();
			ClearAdvice(AdviceType::ADVICE_NONE);
	...
	{
		mNextSurvivalStageCounter = 500;
		PopString aMessage = TodReplaceNumberString("[ADVICE_MORE_IZOMBIE]", "{STREAK}", mChallenge->mSurvivalStage + 1);
		PuzzleSaveStreak();
		ClearAdvice(AdviceType::ADVICE_NONE);
		DisplayAdvice(aMessage, MessageStyle::MESSAGE_STYLE_BIG_MIDDLE, AdviceType::ADVICE_NONE);
		return;
	...
	else
	{
		...
		mApp->PlaySample(PopLib::SOUND_HUGE_WAVE);
		for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++)
		{
...
void Board::DisplayAdvice(const PopString& theAdvice, MessageStyle theMessageStyle, AdviceType theHelpIndex)
{
...
//0x40CA10
void Board::DisplayAdviceAgain(const PopString& theAdvice, MessageStyle theMessageStyle, AdviceType theHelpIndex)
{
	if (theHelpIndex != AdviceType::ADVICE_NONE)
	{
...
void Board::UpdateCursor()
{
	...
	{
		mApp->SetCursor(PopLib::CURSOR_POINTER);
		return;
	}
	...
	{
		mApp->SetCursor(PopLib::CURSOR_DRAGGING);
	}
	else if (aShowFinger)
	{
		mApp->SetCursor(PopLib::CURSOR_HAND);
	}
	else if (aHideCursor)
	{
		mApp->SetCursor(PopLib::CURSOR_NONE);
	}
	else
	{
		mApp->SetCursor(PopLib::CURSOR_POINTER);
...
void Board::UpdateToolTip()
{
	...
	if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO)
	{
		...
		PopString aZombieName = StrFormat("[%s]", GetZombieDefinition(aZombie->mZombieType).mZombieName);
...
void Board::MouseDownWithPlant(int x, int y, int theClickCount)
{
	...
	if (aReason != PlantingReason::PLANTING_OK)
	{
		...
		{
			PopString aSeedName = Plant::GetNameString(mChallenge->GetArtChallengeSeed(aGridX, aGridY), SeedType::SEED_NONE);
			PopString aMessage = TodReplaceString("[ADVICE_WRONG_ART_TYPE]", "{SEED}", aSeedName);
	...
	{
		mApp->PlaySample(PopLib::SOUND_BOWLING);
	}
...
void Board::PickUpTool(GameObjectType theObjectType)
{
	...
	case GameObjectType::OBJECT_TYPE_FERTILIZER:
		...
		{
			mApp->PlaySample(PopLib::SOUND_BUZZER);
		}
		break;
...
void Board::MouseDown(int x, int y, int theClickCount)
{
	...
	{
		mApp->PlaySample(PopLib::SOUND_GRAVEBUTTON);
	}
...
void Board::MouseUp(int x, int y, int theClickCount)
{
	...
	if (CanInteractWithBoardButtons() && theClickCount > 0)
	{
		if (mMenuButton->IsMouseOver() && !mApp->GetDialog(Dialogs::DIALOG_GAME_OVER) && !mApp->GetDialog(Dialogs::DIALOG_LEVEL_COMPLETE))
		{
			...
			{
				mApp->PlaySample(PopLib::SOUND_PAUSE);
...
void Board::ZombiesWon(Zombie* theZombie)
{
	...
	{
		PopString aFlagStr = mApp->Pluralize(GetSurvivalFlagsCompleted(), "[ONE_FLAG]", "[COUNT_FLAGS]");
	...
	mApp->mMusic->StopAllMusic();
	StopAllZombieSounds();
	mApp->PlaySample(PopLib::SOUND_LOSEMUSIC);
...
void Board::UpdateZombieSpawning()
{
	...
	if (mFinalWaveSoundCounter > 0)
	{
		...
		if (mFinalWaveSoundCounter == 0)
		{
			mApp->PlaySample(PopLib::SOUND_FINALWAVE);
		}
	}
...
void Board::DrawIce(Graphics* g, int theGridY)
{
	...
	int aHeight = PopLib::IMAGE_ICE->GetHeight();
	int aWidth = PopLib::IMAGE_ICE->GetWidth();
	...
	for (int aPosX = aBeginningX; aPosX < BOARD_WIDTH; aPosX += aDeltaX)
	{
		...
		g->DrawImage(PopLib::IMAGE_ICE, aRepeatDstRect, aRepeatSrcRect);
	}
	g->DrawImage(PopLib::IMAGE_ICE_CAP, mIceMinX[theGridY], aPosY);
...
void Board::DrawBackdrop(Graphics* g)
{
	...
	switch (mBackground)
	{
	case BackgroundType::BACKGROUND_1_DAY:				aBgImage = PopLib::IMAGE_BACKGROUND1;						break;
	case BackgroundType::BACKGROUND_2_NIGHT:			aBgImage = PopLib::IMAGE_BACKGROUND2;						break;
	case BackgroundType::BACKGROUND_3_POOL:				aBgImage = PopLib::IMAGE_BACKGROUND3;						break;
	case BackgroundType::BACKGROUND_4_FOG:				aBgImage = PopLib::IMAGE_BACKGROUND4;						break;
	case BackgroundType::BACKGROUND_5_ROOF:				aBgImage = PopLib::IMAGE_BACKGROUND5;						break;
	case BackgroundType::BACKGROUND_6_BOSS:				aBgImage = PopLib::IMAGE_BACKGROUND6BOSS;					break;
	case BackgroundType::BACKGROUND_MUSHROOM_GARDEN:	aBgImage = PopLib::IMAGE_BACKGROUND_MUSHROOMGARDEN;		break;
	case BackgroundType::BACKGROUND_GREENHOUSE:			aBgImage = PopLib::IMAGE_BACKGROUND_GREENHOUSE;			break;
	case BackgroundType::BACKGROUND_ZOMBIQUARIUM:		aBgImage = PopLib::IMAGE_AQUARIUM1;						break;
	...
	{
		g->DrawImage(PopLib::IMAGE_BACKGROUND1UNSODDED, -BOARD_OFFSET, 0);
		int aWidth = TodAnimateCurve(0, 1000, mSodPosition, 0, PopLib::IMAGE_SOD1ROW->GetWidth(), TodCurves::CURVE_LINEAR);
		Rect aSrcRect(0, 0, aWidth, PopLib::IMAGE_SOD1ROW->GetHeight());
		g->DrawImage(PopLib::IMAGE_SOD1ROW, 239 - BOARD_OFFSET, 265, aSrcRect);
	}
	else if (((mLevel == 2 || mLevel == 3) && mApp->IsFirstTimeAdventureMode()) || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED)
	{
		g->DrawImage(PopLib::IMAGE_BACKGROUND1UNSODDED, -BOARD_OFFSET, 0);
		g->DrawImage(PopLib::IMAGE_SOD1ROW, 239 - BOARD_OFFSET, 265);
		int aWidth = TodAnimateCurve(0, 1000, mSodPosition, 0, PopLib::IMAGE_SOD3ROW->GetWidth(), TodCurves::CURVE_LINEAR);
		Rect aSrcRect(0, 0, aWidth, PopLib::IMAGE_SOD3ROW->GetHeight());
		g->DrawImage(PopLib::IMAGE_SOD3ROW, 235 - BOARD_OFFSET, 149, aSrcRect);
	}
	else if (mLevel == 4 && mApp->IsFirstTimeAdventureMode())
	{
		g->DrawImage(PopLib::IMAGE_BACKGROUND1UNSODDED, -BOARD_OFFSET, 0);
		g->DrawImage(PopLib::IMAGE_SOD3ROW, 235 - BOARD_OFFSET, 149);
		int aWidth = TodAnimateCurve(0, 1000, mSodPosition, 0, 773, TodCurves::CURVE_LINEAR);
		Rect aSrcRect(232, 0, aWidth, PopLib::IMAGE_BACKGROUND1->GetHeight());
		g->DrawImage(PopLib::IMAGE_BACKGROUND1, 232 - BOARD_OFFSET, 0, aSrcRect);
	}
	else if (aBgImage)
	{
		if (aBgImage == PopLib::IMAGE_BACKGROUND_MUSHROOMGARDEN || aBgImage == PopLib::IMAGE_BACKGROUND_GREENHOUSE || aBgImage == PopLib::IMAGE_AQUARIUM1)
		{
	...
	if (mTutorialState == TutorialState::TUTORIAL_LEVEL_1_PLANT_PEASHOOTER)
	{
		...
		aClipG.DrawImage(PopLib::IMAGE_SOD1ROW, 239 - BOARD_OFFSET, 265);
	...
	{
		g->DrawImage(PopLib::IMAGE_NIGHT_GRAVE_GRAPHIC, 1092, 40);
	}
}
...
void Board::DrawProgressMeter(Graphics* g)
{
	...
	// ▲ 绘制进度条进度部分的贴图
	// ====================================================================================================
	g->DrawImageCel(PopLib::IMAGE_FLAGMETER, 600, 575, 0);
	int aCelWidth = PopLib::IMAGE_FLAGMETER->GetCelWidth();
	int aCelHeight = PopLib::IMAGE_FLAGMETER->GetCelHeight();
	...
	g->DrawImage(PopLib::IMAGE_FLAGMETER, aDstRect, aSrcRect);
	...
	{
		PopString aMatchStr = StrFormat("%d / %d %s", mChallenge->mChallengeScore, 75, TodStringTranslate("[MATCHES]").c_str());
		TodDrawString(g, aMatchStr, aPosX, 589, PopLib::FONT_DWARVENTODCRAFT12, aColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
	else if (mApp->IsSquirrelLevel())
	{
		PopString aMatchStr = StrFormat("%d / %d %s", mChallenge->mChallengeScore, 7, TodStringTranslate("[SQUIRRELS]").c_str());
		TodDrawString(g, aMatchStr, aPosX, 589, PopLib::FONT_DWARVENTODCRAFT12, aColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
	else if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SLOT_MACHINE)
	{
		int aSunMoney = ClampInt(mSunMoney, 0, 2000);
		PopString aMatchStr = StrFormat("%d / %d %s", aSunMoney, 2000, TodStringTranslate("[SUN]").c_str());
		TodDrawString(g, aMatchStr, aPosX, 589, PopLib::FONT_DWARVENTODCRAFT12, aColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
	else if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM)
	{
		int aSunMoney = ClampInt(mSunMoney, 0, 1000);
		PopString aMatchStr = StrFormat("%d / %d %s", aSunMoney, 1000, TodStringTranslate("[SUN]").c_str());
		TodDrawString(g, aMatchStr, aPosX, 589, PopLib::FONT_DWARVENTODCRAFT12, aColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
	else if (mApp->IsIZombieLevel())
	{
		PopString aMatchStr = StrFormat("%d / %d %s", mChallenge->mChallengeScore, 5, TodStringTranslate("[BRAINS]").c_str());
		TodDrawString(g, aMatchStr, aPosX, 589, PopLib::FONT_DWARVENTODCRAFT12, aColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
	else if (ProgressMeterHasFlags())
	{
		...
		for (int aFlagWave = 1; aFlagWave <= aNumFlagWaves; aFlagWave++)
		{
			...
			// 绘制旗杆
			g->DrawImageCel(PopLib::IMAGE_FLAGMETERPARTS, aPosX, 571, 1, 0);
			// 绘制旗帜
			g->DrawImageCel(PopLib::IMAGE_FLAGMETERPARTS, aPosX, 572 - aHeight, 2, 0);
		}
	}
	...
	// ====================================================================================================
	// 绘制“关卡进程”的小牌子
	g->DrawImage(PopLib::IMAGE_FLAGMETERLEVELPROGRESS, 638, 589);
	...
	g->DrawImageCel(PopLib::IMAGE_FLAGMETERPARTS, aCelWidth - aHeadProgress + 580, 572, 0, 0);
}
...
void Board::DrawHouseDoorBottom(Graphics* g)
{
	switch (mBackground)
	{
	case BackgroundType::BACKGROUND_1_DAY:		g->DrawImage(PopLib::IMAGE_BACKGROUND1_GAMEOVER_INTERIOR_OVERLAY, -126, 225);		break;
	case BackgroundType::BACKGROUND_2_NIGHT:	g->DrawImage(PopLib::IMAGE_BACKGROUND2_GAMEOVER_INTERIOR_OVERLAY, -125, 196);		break;
	case BackgroundType::BACKGROUND_3_POOL:		g->DrawImage(PopLib::IMAGE_BACKGROUND3_GAMEOVER_INTERIOR_OVERLAY, -171, 241);		break;
	case BackgroundType::BACKGROUND_4_FOG:		g->DrawImage(PopLib::IMAGE_BACKGROUND4_GAMEOVER_INTERIOR_OVERLAY, -172, 246);		break;
	default:																													break;
	}
}
...
void Board::DrawHouseDoorTop(Graphics* g)
{
	switch (mBackground)
	{
	case BackgroundType::BACKGROUND_1_DAY:		g->DrawImage(PopLib::IMAGE_BACKGROUND1_GAMEOVER_MASK, -130, 202);		break;
	case BackgroundType::BACKGROUND_2_NIGHT:	g->DrawImage(PopLib::IMAGE_BACKGROUND2_GAMEOVER_MASK, -128, 207);		break;
	case BackgroundType::BACKGROUND_3_POOL:		g->DrawImage(PopLib::IMAGE_BACKGROUND3_GAMEOVER_MASK, -172, 234);		break;
	case BackgroundType::BACKGROUND_4_FOG:		g->DrawImage(PopLib::IMAGE_BACKGROUND4_GAMEOVER_MASK, -173, 133);		break;
	case BackgroundType::BACKGROUND_5_ROOF:		g->DrawImage(PopLib::IMAGE_BACKGROUND5_GAMEOVER_MASK, -220, 81);		break;
	case BackgroundType::BACKGROUND_6_BOSS:		g->DrawImage(PopLib::IMAGE_BACKGROUND6_GAMEOVER_MASK, -220, 81);		break;
	default:																										break;
	}
}
...
void Board::DrawLevel(Graphics* g)
{
	...
	else
	{
		...
		if (mApp->IsSurvivalMode() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND)
		{
			...
			if (aFlags > 0)
			{
				PopString aFlagStr = mApp->Pluralize(aFlags, "[ONE_FLAG]", "[COUNT_FLAGS]");
				PopString aCompletedStr = TodReplaceString("[FLAGS_COMPLETED]", "{FLAGS}", aFlagStr);
		...
		else if (mApp->IsEndlessIZombie(mApp->mGameMode) || mApp->IsEndlessScaryPotter(mApp->mGameMode))
		{
			...
			if (aStreak > 0)
			{
				PopString aStreakStr = TodReplaceNumberString("[ENDLESS_STREAK]", "{STREAK}", aStreak);
	...
	TodDrawString(g, aLevelStr, aPosX, aPosY, PopLib::FONT_HOUSEOFTERROR16, Color(224, 187, 98), DrawStringJustification::DS_ALIGN_RIGHT);
}
...
void Board::DrawZenWheelBarrowButton(Graphics* g, int theOffsetY)
{
	...
	if (aPlant && mCursorObject->mCursorType != CursorType::CURSOR_TYPE_PLANT_FROM_WHEEL_BARROW)
	{
		if (mChallenge->mChallengeState == ChallengeState::STATECHALLENGE_ZEN_FADING)
		{
			g->DrawImage(PopLib::IMAGE_ZEN_WHEELBARROW, aButtonRect.mX - 7, aButtonRect.mY + theOffsetY - 3);
		}
		else
		{
			g->DrawImage(PopLib::IMAGE_ZEN_WHEELBARROW, aButtonRect.mX - 7, aButtonRect.mY + theOffsetY + 4);
		}
...
void Board::DrawZenButtons(Graphics* g)
{
	...
	for (GameObjectType aTool = GameObjectType::OBJECT_TYPE_WATERING_CAN; aTool <= GameObjectType::OBJECT_TYPE_NEXT_GARDEN; aTool = (GameObjectType)(aTool + 1))
	{
		...
		if (aTool == GameObjectType::OBJECT_TYPE_NEXT_GARDEN)
		{
			...
			if (!mMenuButton->mBtnNoDraw)
			{
				g->DrawImage(PopLib::IMAGE_ZEN_NEXTGARDEN, aButtonRect.mX + 2, aButtonRect.mY + aOffsetY);
			}
		}
		else
		{
			...
			if (aTool == GameObjectType::OBJECT_TYPE_WATERING_CAN)
			{
				...
				{
					g->DrawImage(PopLib::IMAGE_WATERINGCANGOLD, aButtonRect.mX - 2, aButtonRect.mY + aOffsetY - 6);
				}
				else
				{
					g->DrawImage(PopLib::IMAGE_WATERINGCAN, aButtonRect.mX - 2, aButtonRect.mY + aOffsetY - 6);
				}
			}
			else if (aTool == GameObjectType::OBJECT_TYPE_FERTILIZER)
			{
				...
				g->DrawImage(PopLib::IMAGE_FERTILIZER, aButtonRect.mX - 6, aButtonRect.mY + aOffsetY - 7);
				g->SetColorizeImages(false);

				PopString aChargeString = StrFormat("x%d", aCharges);
				TodDrawString(g, aChargeString, aButtonRect.mX + 64, aButtonRect.mY + aOffsetY + 65, PopLib::FONT_HOUSEOFTERROR16, Color::White, DS_ALIGN_RIGHT);
			}
			else if (aTool == GameObjectType::OBJECT_TYPE_BUG_SPRAY)
			{
				...
				g->DrawImage(PopLib::IMAGE_BUG_SPRAY, aButtonRect.mX, aButtonRect.mY + aOffsetY - 1);
				g->SetColorizeImages(false);
			...
			{
				g->DrawImage(PopLib::IMAGE_PHONOGRAPH, aButtonRect.mX + 2, aButtonRect.mY + aOffsetY + 2);
			}
			else if (aTool == GameObjectType::OBJECT_TYPE_CHOCOLATE)
			{
				...
				g->DrawImage(PopLib::IMAGE_CHOCOLATE, aButtonRect.mX + 6, aButtonRect.mY + aOffsetY + 4);
				g->SetColorizeImages(false);
			...
			{
				g->DrawImage(PopLib::IMAGE_ZEN_MONEYSIGN, aButtonRect.mX - 5, aButtonRect.mY + aOffsetY - 4);
			}
			...
			else if (aTool == GameObjectType::OBJECT_TYPE_TREE_FOOD)
			{
				...
				g->DrawImage(PopLib::IMAGE_TREEFOOD, aButtonRect.mX - 6, aButtonRect.mY + aOffsetY - 7);
				g->SetColorizeImages(false);
	...
	if (mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN && mApp->mGameMode != GameMode::GAMEMODE_TREE_OF_WISDOM)
	{
		if (mShowShovel)
		{
			...
			if (mCursorObject->mCursorType != CursorType::CURSOR_TYPE_SHOVEL)
			{
				...
				g->DrawImage(PopLib::IMAGE_SHOVEL, aShovelRect.mX - 7, aShovelRect.mY - 3);
				g->SetColorizeImages(false);
			}
		}
	}
	...
	case DebugTextMode::DEBUG_TEXT_MUSIC:
	{
		...
		else
		{
				...
				mApp->mMusic->mCurMusicTune == MusicTune::MUSIC_TUNE_ROOF_GRAZETHEROOF)
			{
				...
				aText += StrFormat("bpm1 %d bmp2 %d bpm3 %d\n", bpm1, bpm2, bpm3);
			}
			else if (mApp->mMusic->mCurMusicTune == MusicTune::MUSIC_TUNE_NIGHT_MOONGRAINS)
			{
				int aPackedOrderDrums = mApp->mMusic->GetMusicOrder(mApp->mMusic->mCurMusicFileDrums);
...
void Board::DrawUIBottom(Graphics* g)
{
	if (mBackground == BackgroundType::BACKGROUND_ZOMBIQUARIUM)
	{
		...
		g->DrawImageCel(PopLib::IMAGE_WAVESIDE, 0, 40, aWaveTime);
		g->DrawImageCel(PopLib::IMAGE_WAVECENTER, 160, 40, aWaveTime);
		g->DrawImageCel(PopLib::IMAGE_WAVECENTER, 320, 40, aWaveTime);
		g->DrawImageCel(PopLib::IMAGE_WAVECENTER, 480, 40, aWaveTime);
		TodDrawImageCelScaled(g, PopLib::IMAGE_WAVESIDE, 800, 40, 0, aWaveTime, -1.0f, 1.0f);
...
void Board::DrawUICoinBank(Graphics* g)
{
	...
	int aPosX = 57;
	int aPosY = 599 - PopLib::IMAGE_COINBANK->GetHeight();
	...
	g->SetColor(Color(255, 255, 255, anAlpha));
	g->DrawImage(PopLib::IMAGE_COINBANK, aPosX, aPosY);

	g->SetColor(Color(180, 255, 90, anAlpha));
	g->SetFont(PopLib::FONT_CONTINUUMBOLD14);
	PopString aCoinLabel = mApp->GetMoneyString(mApp->mPlayerInfo->mCoins);
	g->DrawString(aCoinLabel, aPosX + 116 - PopLib::FONT_CONTINUUMBOLD14->StringWidth(aCoinLabel), aPosY + 24);
	g->SetColorizeImages(false);
}
...
{
	Image* aImageFog = mApp->Is3DAccelerated() ? PopLib::IMAGE_FOG : PopLib::IMAGE_FOG_SOFTWARE;
	for (int x = 0; x < MAX_GRID_SIZE_X; x++)
	{
		for (int y = 0; y < MAX_GRID_SIZE_Y + 1; y++)
		{
			...
			if (!mApp->Is3DAccelerated())
			{
				aPosX += 10;
				aPosY += 3;
				aCelCol = aCelLook % PopLib::IMAGE_FOG_SOFTWARE->mNumCols;
				aColorVariant = 255;
...
{
	mApp->PlaySample(PopLib::SOUND_BOING);
...
{
	mApp->PlaySample(PopLib::SOUND_SUKHBIR);
