...
void DrawSeedPacket(Graphics* g, float x, float y, SeedType theSeedType, SeedType theImitaterType, float thePercentDark, int theGrayness, bool theDrawCost, bool theUseCurrentCost)
{
	...
	if (g->mScaleX > 1)
	{
		TodDrawImageCelScaledF(g, PopLib::IMAGE_SEEDPACKET_LARGER, x, y, 0, 0, g->mScaleX * 0.5f, g->mScaleY * 0.5f);
	}
	else
	{
		TodDrawImageCelScaledF(g, PopLib::IMAGE_SEEDS, x, y, aPacketBackground, 0, g->mScaleX, g->mScaleY);
	}
	...
	if (thePercentDark > 0.0f)
	{
		...
		TodDrawImageCelScaledF(&aPlantG, PopLib::IMAGE_SEEDS, x, y, aPacketBackground, 0, aPlantG.mScaleX, aPlantG.mScaleY);
		if (aDrawSeedInMiddle)
		{
	...
	if (theDrawCost)
	{
		...
		Font* aTextFont = PopLib::FONT_PICO129;
...
void SeedBank::Draw(Graphics* g)
{
	...
	if (!mBoard->HasConveyorBeltSeedBank())
	{
		PopString aMoneyLabel = StrFormat("%d", std::max(mBoard->mSunMoney, 0));
