...
void SaveGameContext::SyncImage(Image*& theImage)
{
	if (mReading)
	{
		ResourceId aResID;
		SyncInt((int&)aResID);
		if (aResID == PopLib::ResourceId::RESOURCE_ID_MAX)
		{
			theImage = nullptr;
		}
		else
	...
	else
	{
		...
		{
			aResID = PopLib::ResourceId::RESOURCE_ID_MAX;
		}
		SyncInt((int&)aResID);
	}
}
...
bool LawnLoadGame(Board* theBoard, const std::string& theFilePath)
{
	...
	if (!gAppBase->ReadBufferFromFile(theFilePath, &aContext.mBuffer))
	{
		return false;
	}
...
bool LawnSaveGame(Board* theBoard, const std::string& theFilePath)
{
	...
	aContext.SyncBytes(&aHeader, sizeof(aHeader));
	SyncBoard(aContext, theBoard);
	return gAppBase->WriteBufferToFile(theFilePath, &aContext.mBuffer);
}
