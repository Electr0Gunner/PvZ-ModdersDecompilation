...
void ProfileMgr::Load()
{
    ...
    try
    {
        if (gAppBase->ReadBufferFromFile(aFileName, &aBuffer))
        {
            DataReader aReader;
...
void ProfileMgr::Save()
{
    ...
    std::string aFileName = GetAppDataFolder() + "userdata/users.dat";
    gAppBase->WriteBytesToFile(aFileName, aWriter.GetDataPtr(), aWriter.GetDataLen());
}
...
//0x46AF70
bool ProfileMgr::DeleteProfile(const PopString& theName)
{
...
//0x46AFF0
bool ProfileMgr::RenameProfile(const PopString& theOldName, const PopString& theNewName)
{
...
//0x46B290
PlayerInfo* ProfileMgr::GetProfile(const PopString& theName)
{
    auto anItr = mProfileMap.find(theName);
    if (anItr != mProfileMap.end())
    {
...
//0x46B310
PlayerInfo* ProfileMgr::AddProfile(const PopString& theName)
{
