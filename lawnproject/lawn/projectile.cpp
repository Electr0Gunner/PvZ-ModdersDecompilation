...
void Projectile::UpdateNormalMotion()
{
	...
	else if (mMotionType == ProjectileMotion::MOTION_HOMING)
	{
		...
		{
			Rect aZombieRect = aZombie->GetZombieRect();
			Vector2 aTargetCenter(aZombie->ZombieTargetLeadX(0.0f), aZombieRect.mY + aZombieRect.mHeight / 2);
			Vector2 aProjectileCenter(mPosX + mWidth / 2, mPosY + mHeight / 2);
			Vector2 aToTarget = (aTargetCenter - aProjectileCenter).Normalize();
			Vector2 aMotion(mVelX, mVelY);
