...
bool TodParticleLoadADef(TodParticleDefinition* theParticleDef, const char* theParticleFileName)
{
	...
	else
	{
		for (int i = 0; i < theParticleDef->mEmitterDefCount; i++)
		{
			...
			//if (aDef.mImage)
				//((MemoryImage*)aDef.mImage)->mImageFlags |= D3DImageFlags::D3DImageFlag_MinimizeNumSubdivisions; TODO: POSSIBLY ADD THIS WITH SDL RENDERER OVERHAUL
		}
		return true;
	}
}
...
void TodParticleLoadDefinitions(ParticleParams* theParticleParamArray, int theParticleParamArraySize)
{
	...
	for (int i = 0; i < gParticleParamArraySize; i++)
	{
		...
		gAppBase->mNumLoadingThreadTasks += 6;
	}
}
...
void TodParticleEmitter::TodEmitterInitialize(float theX, float theY, TodParticleSystem* theSystem, TodEmitterDefinition* theEmitterDef)
{
	...
	mSystemAge = -1;
	mDead = false;
	mColorOverride = PopLib::Color::White;
	...
	if (FloatTrackIsSet(mEmitterDef->mSystemDuration))
		mSystemDuration = FloatTrackEvaluate(mEmitterDef->mSystemDuration, 0.0f, PopLib::Rand(1.0f));
	else
	...
	{
		mSystemFieldInterp[i][0] = PopLib::Rand(1.0f);
		mSystemFieldInterp[i][1] = PopLib::Rand(1.0f);
	}
	for (int j = 0; j < 10; j++)
		mTrackInterp[j] = PopLib::Rand(1.0f);
...
TodParticle* TodParticleEmitter::SpawnParticle(int theIndex, int theSpawnCount)
{
	...
	{
		aParticle->mParticleFieldInterp[i][0] = PopLib::Rand(1.0f);  // 初始化每个粒子场的横向插值
		aParticle->mParticleFieldInterp[i][1] = PopLib::Rand(1.0f);  // 初始化每个粒子场的纵向插值
	}
	for (int i = 0; i < (int)ParticleTracks::NUM_PARTICLE_TRACKS; i++)
		aParticle->mParticleInterp[i] = PopLib::Rand(1.0f);  // 初始化每条通道的插值

	float aParticleDurationInterp = PopLib::Rand(1.0f);
	float aLaunchSpeedInterp = PopLib::Rand(1.0f);
	float aEmitterOffsetXInterp = PopLib::Rand(1.0f);
	float aEmitterOffsetYInterp = PopLib::Rand(1.0f);
	...
	if (TestBit(mEmitterDef->mParticleFlags, (int)ParticleFlags::PARTICLE_RANDOM_START_TIME))
		aParticle->mParticleAge = PopLib::Rand(aParticle->mParticleDuration);  // 对于“随机初始时间”的粒子
	float aLaunchSpeed = FloatTrackEvaluate(mEmitterDef->mLaunchSpeed, mSystemTimeValue, aLaunchSpeedInterp) * 0.01f;
	float aLaunchAngleInterp = PopLib::Rand(1.0f);
	...
	if (mEmitterDef->mEmitterType == EmitterType::EMITTER_CIRCLE_PATH)
	{
		...
		aLaunchAngle = PopLib::Rand((float)(2 * PI));
	else
		// 其他情况下，根据发射角度的定义值计算
		aLaunchAngle = DEG_TO_RAD(FloatTrackEvaluate(mEmitterDef->mLaunchAngle, mSystemTimeValue, aLaunchAngleInterp));
	...
	{
		float aEmitterRadiusInterp = PopLib::Rand(1.0f);
	...
	{
		float aEmitterBoxXInterp = PopLib::Rand(1.0f);
		float aEmitterBoxYInterp = PopLib::Rand(1.0f);
	...
	float aEmitterSkewXInterp = PopLib::Rand(1.0f);
	float aEmitterSkewYInterp = PopLib::Rand(1.0f);
	...
	if (mEmitterDef->mAnimated || FloatTrackIsSet(mEmitterDef->mAnimationRate))
		...
		aParticle->mImageFrame = PopLib::Rand(mEmitterDef->mImageFrames);  // 对于帧固定的粒子，在贴图的所有帧中随机取得一帧，后续一般不再变化

	if (TestBit(mEmitterDef->mParticleFlags, (int)ParticleFlags::PARTICLE_RANDOM_LAUNCH_SPIN))
		aParticle->mSpinPosition = PopLib::Rand((float)(2 * PI));  // 在 [0, 2π] 之间随机取得一个初始旋转角度
...
void TodParticleEmitter::UpdateParticleField(TodParticle* theParticle, ParticleField* theParticleField, float theParticleTimeValue, int theFieldIndex)
{
	...
	case ParticleFieldType::FIELD_CIRCLE:  // 圆周
	{
		...
		Vector2 aMotion = aToCenter.Perp().Normalize();  // 标准化的法向量
	...
	{
		Vector2 aToCenter = theParticle->mPosition - mSystemCenter;
		Vector2 aMotion = aToCenter.Normalize();  // 标准化的方向向量
		float aRadius = aToCenter.Magnitude();
		aMotion *= 0.01 * (x + aRadius * y);
		theParticle->mPosition += aMotion;
		break;
...
bool TodParticleEmitter::CrossFadeParticle(TodParticle* theParticle, TodParticleEmitter* theToEmitter)
{
	...
	{
		float aCrossFadeDurationInterp = PopLib::Rand(1);
...
{
	bool aHardWare = gAppBase->Is3DAccelerated();
...
void TodParticleEmitter::CrossFadeEmitter(TodParticleEmitter* theToEmitter)
{
	...
	float aCrossFadeDurationInterp = PopLib::Rand(1.0f);
