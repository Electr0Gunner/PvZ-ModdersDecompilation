...
#include "todlib/toddrawtriangle/TodDrawTriangleInc.cpp"
...
void TodTriangleGroup::DrawGroup(Graphics* g)
{
	...
	{
		if (!gAppBase->Is3DAccelerated() && mDrawMode == Graphics::DRAWMODE_ADDITIVE)
...
//0x446300
void TodTriangleGroup::AddTriangle(Graphics* g, Image* theImage, const Matrix3& theMatrix, const Rect& theClipRect, const Color& theColor, int theDrawMode, const Rect& theSrcRect)
{
	...
	{
		if ((
				theClipRect.mX == 0 && theClipRect.mY == 0 && theClipRect.mWidth == BOARD_WIDTH && theClipRect.mHeight == BOARD_HEIGHT && gAppBase->Is3DAccelerated()
			) || (
