...
int TodPickFromArray(const int* theArray, int theCount)
{
	...
	return theCount > 0 ? theArray[PopLib::Rand(theCount)] : 0;
}
...
TodWeightedArray* TodPickArrayItemFromWeightedArray(const TodWeightedArray* theArray, int theCount)
{
	...
	aTotalWeight = PopLib::Rand(aTotalWeight);
...
{
	PopString aFinalString = TodStringTranslate(theText);
...
{
	PopString aFinalString = TodStringTranslate(theString);
	...
	{
		PopChar aChar = aFont->GetMappedChar(aFinalString[aCharNum]);
		PopChar aNextChar = '\0';
	...
	for (int aPoolIdx = 0; aPoolIdx < 256; aPoolIdx++)
	{
		...
		while (aRenderCommand)
		{
			...
			if (aRenderCommand->mImage)
			{
				...
				Matrix3Translation(aTransform, aPosX, aPosY);
				Matrix3Multiply(aTransform, theMatrix, aTransform);
...
void Matrix3Translation(Matrix3& m, float x, float y)
{
	m.m02 += x;
	m.m12 += y;
}

void TodScaleTransformMatrix(Matrix3& m, float x, float y, float theScaleX, float theScaleY)
{
	m.m00 = theScaleX;
	m.m10 = 0.0f;
	m.m20 = 0.0f;
...
{
	((MemoryImage*)theImage)->mImageFlags |= D3DIMAGEFLAG_SANDING;
}

void TodSandImageIfNeeded(Image* theImage)
{
	MemoryImage* aImage = (MemoryImage*)theImage;
	if (TestBit(aImage->mImageFlags, D3DIMAGEFLAG_SANDING))
	{
		FixPixelsOnAlphaEdgeForBlending(theImage);
		//((MemoryImage*)theImage)->mD3DFlags &= ~D3DIMAGEFLAG_SANDING;
		SetBit((unsigned int&)aImage->mImageFlags, D3DIMAGEFLAG_SANDING, false);  // 清除标记
	}
}
...
void Matrix3Transpose(const Matrix3& m, Matrix3 &r)
{
	Matrix3 temp;
	temp.m00 = m.m00;
	temp.m01 = m.m10;
...
//0x512D00
void Matrix3Inverse(const Matrix3& m, Matrix3 &r)
{
...
//0x512E20
void Matrix3Multiply(Matrix3& m, const Matrix3& l, const Matrix3& r)
{
	//Matrix3 temp = l * r;
	Matrix3 temp;
...
{
	return ((TodResourceManager*)gAppBase->mResourceManager)->TodLoadResources(theGroup);
}
...
bool TodResourceManager::TodLoadResources(const std::string& theGroup)
{
	...
	while (!gAppBase->mShutdown && TodLoadNextResource());
	if (gAppBase->mShutdown)
		return false;

	if (HadError())
	{
		gAppBase->ShowResourceError(true);
		return false;
	}
	...
	int aDuration = fmax(aTimer.GetDuration(), 0);
	if (aDuration > 20)
	{
...
{ 
	((TodResourceManager*)gAppBase->mResourceManager)->AddImageToMap(theImage, thePath);
}
...
{
	return ((TodResourceManager*)gAppBase->mResourceManager)->TodLoadNextResource();
}
...
PopString TodReplaceString(const PopString& theText, const PopChar* theStringToFind, const PopString& theStringToSubstitute)
{
	...
	if (aPos != PopString::npos)
	{
		PopString aFinalStringToSubstitute = TodStringTranslate(theStringToSubstitute);
...
PopString TodReplaceNumberString(const PopString& theText, const PopChar* theStringToFind, int theNumber)
{
	...
	if (aPos != PopString::npos)
	{
		PopString aNumberString = StrFormat("%d", theNumber);
...
bool TodIsPointInPolygon(const Vector2* thePolygonPoint, int theNumberPolygonPoints, const Vector2& theCheckPoint)
{
	...
	{
		const Vector2& cur = thePolygonPoint[i];
		const Vector2& nex = thePolygonPoint[i == theNumberPolygonPoints - 1 ? 0 : i + 1];

		Vector2 u = (nex - cur).Perp();
		Vector2 v = theCheckPoint - cur;
		if (u.Dot(v) < 0)
			return false;
	}
	return true;
