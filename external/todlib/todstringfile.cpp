...
bool TodStringListReadName(const char*& thePtr, std::string& theName)
{
	...
	else
	{
		...
		theName = PopLib::Trim(std::string(aNameStart + 1, aCount));  // 取得中括号之间的部分并去除字符串前后的空白字符
		if (theName.size() == 0)
		{
...
bool TodStringListReadValue(const char*& thePtr, std::string& theValue)
{
	...
	theValue = PopLib::Trim(std::string(thePtr, aLen));  // 如果存在下一个“[”，则取到“[”前为止；否则，取剩下的全部
...
bool TodStringListReadItems(const char* theFileText)
{
	...
	for (;;)
	{
		...
		std::string aNameUpper = PopLib::StringToUpper(aName);
		gAppBase->SetString(aNameUpper, aValue);
	}
}
...
void TodStringListLoad(const char* theFileName)
{
	if (!TodStringListReadFile(theFileName))
		TodErrorMessageBox(PopLib::StrFormat("Failed to load string list file '%s'", theFileName).c_str(), "Error");
}

//0x519410
PopString TodStringListFind(const PopString& theName)
{
	StringStringMap::iterator anItr = gAppBase->mStringProperties.find(theName);
	if (anItr != gAppBase->mStringProperties.end())
	{
		return anItr->second;
	}
	else
	{
		return PopLib::StrFormat("<Missing %s>", theName.c_str());
	}
}
...
PopString TodStringTranslate(const PopString& theString)
{
	...
	{
		PopString aName = theString.substr(1, theString.size() - 2);  // 取“[”与“]”中间的部分
...
PopString TodStringTranslate(const PopChar* theString)
{
	if (theString != nullptr)
	{
		...
		{
			PopString aName(theString, 1, aLen - 2);  // 取“[”与“]”中间的部分
			return TodStringListFind(aName);
		}
		else
			return theString;
...
bool TodStringListExists(const PopString& theString)
{
	...
	{
		PopString aName = theString.substr(1, theString.size() - 2);  // 取“[”与“]”中间的部分
		return gAppBase->mStringProperties.find(aName) != gAppBase->mStringProperties.end();
	}
	return false;
}
...
int TodWriteWordWrappedHelper(Graphics* g, const PopString& theString, int theX, int theY, TodStringListFormat& theCurrentFormat, int theWidth, DrawStringJustification theJustification, bool drawString, int theOffset, int theLength, int theMaxChars)
{
...
//0x51A040
void TodDrawStringWrapped(Graphics* g, const PopString& theText, const Rect& theRect, Font* theFont, const Color& theColor, DrawStringJustification theJustification)
{
	PopString aTextFinal = TodStringTranslate(theText);
