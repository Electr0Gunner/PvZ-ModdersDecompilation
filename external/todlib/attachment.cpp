...
//0x404610
void Attachment::SetPosition(const Vector2& thePosition)
{
	TOD_ASSERT(gEffectSystem);
	...
	{
		AttachEffect* aAttachEffect = &mEffectArray[i];
		Vector2 aNewPos = aAttachEffect->mOffset * thePosition;
...
//0x404B80
void Attachment::SetMatrix(const Transform2D& theMatrix)
{
	TOD_ASSERT(gEffectSystem);
	...
	{
		AttachEffect* aAttachEffect = &mEffectArray[i];
		Transform2D aPosition = theMatrix * aAttachEffect->mOffset;
		...
		case EffectType::EFFECT_TRAIL:
		{
			...
			if (aTrail)
			{
				aTrail->mTrailCenter = Vector2(aPosition.m02, aPosition.m12);
			}
			break;
		}
...
//0x4051B0
void AttachmentUpdateAndSetMatrix(AttachmentID& theAttachmentID, Transform2D& theMatrix)
{
...
void AttachmentUpdateAndMove(AttachmentID& theAttachmentID, float theX, float theY)
{
	...
	{
		aAttachment->Update();
		aAttachment->SetPosition(Vector2(theX, theY));
	}
	else
	{
		theAttachmentID = AttachmentID::ATTACHMENTID_NULL;
