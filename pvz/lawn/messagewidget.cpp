...
//0x459010
void MessageWidget::SetLabel(const PopString& theNewLabel, MessageStyle theMessageStyle)
{
	PopString aLabel = TodStringTranslate(theNewLabel);
...
void MessageWidget::LayoutReanimText()
{
	...
	for (int aPos = 0; aPos <= aLabelLen; aPos++)
	{
		if (aPos == aLabelLen || mLabel[aPos] == '\n')
		{
			...
			int aOff = aCurPos;
			aCurPos = aPos + 1;
			PopString aLine(&mLabel[aOff], aLen);
...
Font* MessageWidget::GetFont()
{
	...
	case MessageStyle::MESSAGE_STYLE_HUGE_WAVE:
	case MessageStyle::MESSAGE_STYLE_ZEN_GARDEN_LONG:
		return PopLib::FONT_HOUSEOFTERROR28;

	case MessageStyle::MESSAGE_STYLE_SLOT_MACHINE:
		return PopLib::FONT_HOUSEOFTERROR16;
	}
...
void MessageWidget::Draw(Graphics* g)
{
	...
	bool aFadeOut = false;
	if (aFont == PopLib::FONT_CONTINUUMBOLD14)
	{
		aOutlineFont = PopLib::FONT_CONTINUUMBOLD14OUTLINE;
	}
	...
	if (mReanimType != ReanimationType::REANIM_NONE)
	{
		if (aFont == PopLib::FONT_CONTINUUMBOLD14)
		{
			DrawReanimatedText(g, PopLib::FONT_CONTINUUMBOLD14OUTLINE, Color::Black, aPosY);
		}
	...
	else
	{
		...
		if (mMessageStyle == MessageStyle::MESSAGE_STYLE_HOUSE_NAME)
		{
			...
			if (mApp->IsSurvivalMode() && mApp->mBoard->mChallenge->mSurvivalStage > 0)
			{
				...
				PopString aSubStr = TodReplaceString("[FLAGS_COMPLETED]", "{FLAGS}", aFlagStr);
			}
