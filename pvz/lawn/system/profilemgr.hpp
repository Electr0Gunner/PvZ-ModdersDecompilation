...
class DataSync;
class PlayerInfo;
typedef std::pair<PopString, PlayerInfo> ProfilePair;
typedef std::map<PopString, PlayerInfo, PopLib::StringLessNoCase> ProfileMap;
...
public:
	bool				DeleteProfile(const PopString& theName);
...
public:
	...
	PlayerInfo*			GetProfile(const PopString& theName);
	PlayerInfo*			AddProfile(const PopString& theName);
	PlayerInfo*			GetAnyProfile();
	bool				RenameProfile(const PopString& theOldName, const PopString& theNewName);
