...
StoreScreen::StoreScreen(LawnApp* theApp) : Dialog(nullptr, nullptr, DIALOG_STORE, true, "Store", "", "", BUTTONS_NONE)
{
    ...
    Image* aMenuImage = PopLib::IMAGE_STORE_MAINMENUBUTTON;
    mBackButton->mButtonImage = aMenuImage;
    mBackButton->mOverImage = PopLib::IMAGE_STORE_MAINMENUBUTTONHIGHLIGHT;
    mBackButton->mDownImage = PopLib::IMAGE_STORE_MAINMENUBUTTONDOWN;
    mBackButton->SetFont(PopLib::FONT_HOUSEOFTERROR20);
    ...
    Image* aPrevImage = PopLib::IMAGE_STORE_PREVBUTTON;
    mPrevButton->mButtonImage = aPrevImage;
    mPrevButton->mOverImage = PopLib::IMAGE_STORE_PREVBUTTONHIGHLIGHT;
    mPrevButton->mDownImage = PopLib::IMAGE_STORE_PREVBUTTONHIGHLIGHT;
    ...
    Image* aNextImage = PopLib::IMAGE_STORE_NEXTBUTTON;
    mNextButton->mButtonImage = aNextImage;
    mNextButton->mOverImage = PopLib::IMAGE_STORE_NEXTBUTTONHIGHLIGHT;
    mNextButton->mDownImage = PopLib::IMAGE_STORE_NEXTBUTTONHIGHLIGHT;
    ...
    {
        mPrevButton->mDisabledImage = PopLib::IMAGE_STORE_PREVBUTTONDISABLED;
        mPrevButton->SetDisabled(true);
        mNextButton->mDisabledImage = PopLib::IMAGE_STORE_NEXTBUTTONDISABLED;
...
void StoreScreen::DrawItemIcon(Graphics* g, int theItemPosition, StoreItem theItemType, bool theIsForHighlight)
{
    ...
    if (theItemType == STORE_ITEM_PACKET_UPGRADE)
    {
        ...
        g->DrawImage(PopLib::IMAGE_STORE_PACKETUPGRADE, aPosX - 7, aPosY + 7);
        if (theIsForHighlight)
        {
        ...
        PopString aSlotText = TodReplaceNumberString("[STORE_UPGRADE_SLOTS]", "{SLOTS}", mApp->mPlayerInfo->mPurchases[STORE_ITEM_PACKET_UPGRADE] + 7);
        Rect aRect(aPosX, aPosY + 6, 55, 70);
        TodDrawStringWrapped(g, aSlotText, aRect, PopLib::FONT_HOUSEOFTERROR16, Color::White, DS_ALIGN_CENTER_VERTICAL_MIDDLE);
    }
    else if (theItemType == STORE_ITEM_POOL_CLEANER)
    {
        g->DrawImage(PopLib::IMAGE_ICON_POOLCLEANER, aPosX + 1, aPosY + 7);
    }
    else if (theItemType == STORE_ITEM_RAKE)
    {
        g->DrawImage(PopLib::IMAGE_ICON_RAKE, aPosX - 5, aPosY + 10);
    }
    else if (theItemType == STORE_ITEM_ROOF_CLEANER)
    {
        g->DrawImage(PopLib::IMAGE_ICON_ROOFCLEANER, aPosX, aPosY + 28);
    }
    else if (theItemType == STORE_ITEM_PLANT_IMITATER)
    {
        g->DrawImage(PopLib::IMAGE_IMITATERSEED, aPosX, aPosY);
    }
    else if (theItemType == STORE_ITEM_MUSHROOM_GARDEN)
    {
        g->DrawImage(PopLib::IMAGE_STORE_MUSHROOMGARDENICON, aPosX - 8, aPosY + 2);
    }
    else if (theItemType == STORE_ITEM_AQUARIUM_GARDEN)
    {
        g->DrawImage(PopLib::IMAGE_STORE_AQUARIUMGARDENICON, aPosX - 8, aPosY + 2);
    }
    else if (theItemType == STORE_ITEM_TREE_OF_WISDOM)
    {
        g->DrawImage(PopLib::IMAGE_STORE_TREEOFWISDOMICON, aPosX - 8, aPosY + 2);
    }
    else if (theItemType == STORE_ITEM_FIRSTAID)
    {
        g->DrawImage(PopLib::IMAGE_STORE_FIRSTAIDWALLNUTICON, aPosX - 1, aPosY + 13);
    }
    else if (theItemType == STORE_ITEM_PVZ)
    {
        g->DrawImage(PopLib::IMAGE_STORE_PVZICON, aPosX, aPosY - 9);
    }
    else if (theItemType == STORE_ITEM_TREE_FOOD)
    {
        g->DrawImage(PopLib::IMAGE_TREEFOOD, aPosX - 8, aPosY - 2);
    }
    else if (theItemType == STORE_ITEM_STINKY_THE_SNAIL)
    {
        g->DrawImage(PopLib::IMAGE_REANIM_STINKY_TURN3, aPosX - 24, aPosY + 14);
    }
    else if (theItemType == STORE_ITEM_GOLD_WATERINGCAN)
    {
        g->DrawImage(PopLib::IMAGE_WATERINGCANGOLD, aPosX - 14, aPosY - 4);
    }
    else if (theItemType == STORE_ITEM_FERTILIZER)
    {
        g->DrawImage(PopLib::IMAGE_FERTILIZER, aPosX - 11, aPosY - 2);
        TodDrawString(g, "x5", aPosX + 56, aPosY + 62, PopLib::FONT_HOUSEOFTERROR16, Color::White, DS_ALIGN_RIGHT);
    }
    else if (theItemType == STORE_ITEM_PHONOGRAPH)
    {
        g->DrawImage(PopLib::IMAGE_PHONOGRAPH, aPosX - 12, aPosY + 3);
    }
    else if (theItemType == STORE_ITEM_BUG_SPRAY)
    {
        g->DrawImage(PopLib::IMAGE_BUG_SPRAY, aPosX - 12, aPosY + 3);
        TodDrawString(g, "x5", aPosX + 56, aPosY + 62, PopLib::FONT_HOUSEOFTERROR16, Color::White, DS_ALIGN_RIGHT);
    }
    else if (theItemType == STORE_ITEM_GARDENING_GLOVE)
    {
        g->DrawImage(PopLib::IMAGE_ZEN_GARDENGLOVE, aPosX - 12, aPosY + 3);
    }
    else if (theItemType == STORE_ITEM_WHEEL_BARROW)
    {
        g->DrawImage(PopLib::IMAGE_ZEN_WHEELBARROW, aPosX - 12, aPosY + 3);
    }
...
void StoreScreen::DrawItem(Graphics* g, int theItemPosition, StoreItem theItemType)
{
    ...
    {
        g->DrawImage(PopLib::IMAGE_STORE_PRICETAG, aPosX - 3, aPosY + 70);
        PopString aCostString = LawnApp::GetMoneyString(GetItemCost(theItemType));
        TodDrawString(g, aCostString, aPosX + 23, aPosY + 85, PopLib::FONT_BRIANNETOD12, Color::Black, DS_ALIGN_CENTER);
    }
    if (IsComingSoon(theItemType))
    {
        ...
        TodDrawStringWrapped(g, "[COMING_SOON]", aRect, PopLib::FONT_HOUSEOFTERROR16, Color(255, 0, 0), DS_ALIGN_CENTER_VERTICAL_MIDDLE);
    }
    else if (IsItemSoldOut(theItemType))
    {
        Rect aRect(aPosX, aPosY, 50, 70);
        TodDrawStringWrapped(g, "[SOLD_OUT]", aRect, PopLib::FONT_HOUSEOFTERROR16, Color(255, 0, 0), DS_ALIGN_CENTER_VERTICAL_MIDDLE);
    }
    else if (mMouseOverItem == theItemType)
    {
        if (theItemType >= 0 && theItemType <= 8)
        {
            g->DrawImage(PopLib::IMAGE_SEEDPACKETFLASH, aPosX, aPosY);
        }
        else
        {
...
void StoreScreen::Draw(Graphics* g)
{
    ...
    if (mApp->IsNight())
    {
        g->DrawImage(PopLib::IMAGE_STORE_BACKGROUNDNIGHT, 0, 0);
    }
    else
    {
        g->DrawImage(PopLib::IMAGE_STORE_BACKGROUND, 0, 0);
    }

    if (!mHatchTimer && mHatchOpen)
    {
        g->DrawImage(PopLib::IMAGE_STORE_CAR, mShakeX + 196, mShakeY + 138);
        g->DrawImage(PopLib::IMAGE_STORE_HATCHBACKOPEN, mShakeX + 299, mShakeY);
        if (mApp->IsNight())
        {
            g->DrawImage(PopLib::IMAGE_STORE_CAR_NIGHT, mShakeX + 688, mShakeY + 193);
        }
    }
    else
    {
        g->DrawImage(PopLib::IMAGE_STORE_CARCLOSED, mShakeX + 196, mShakeY + 138);
        if (mApp->IsNight())
        {
            g->DrawImage(PopLib::IMAGE_STORE_CAR_NIGHT, mShakeX + 688, mShakeY + 193);
            g->DrawImage(PopLib::IMAGE_STORE_CARCLOSED_NIGHT, mShakeX + 337, mShakeY + 187);
        }
    }
    g->DrawImage(PopLib::IMAGE_STORE_SIGN, 285, aStoreSignPosY);
    ...
    g->DrawImage(PopLib::IMAGE_COINBANK, STORESCREEN_COINBANK_X, STORESCREEN_COINBANK_Y);
    ...
    g->DrawString(aCoinLabel, STORESCREEN_COINBANK_X + 111 - PopLib::FONT_CONTINUUMBOLD14->StringWidth(aCoinLabel), STORESCREEN_COINBANK_Y + 24);
    ...
    if (!mPrevButton->mDisabled)
    {
        ...
        PopString aPageString = TodReplaceNumberString(TodReplaceNumberString("[STORE_PAGE]", "{PAGE}", mPage), "{NUM_PAGES}", aNumPages);
        TodDrawString(g, aPageString, STORESCREEN_PAGESTRING_X, STORESCREEN_COINBANK_Y, PopLib::FONT_BRIANNETOD12, Color(80, 80, 80), DS_ALIGN_CENTER);
    }
}
...
void StoreScreen::ButtonPress(int theId)
{
    if (theId != StoreScreen::StoreScreen_Prev && theId != StoreScreen::StoreScreen_Next)
        mApp->PlaySample(PopLib::SOUND_BUTTONCLICK);
}
...
void StoreScreen::ButtonDepress(int theId)
{
    ...
    {
        mHatchTimer = 50;
        mApp->PlaySample(PopLib::SOUND_HATCHBACK_CLOSE);
        mBubbleCountDown = 0;
        mApp->CrazyDaveStopTalking();
        EnableButtons(false);
        do
...
void StoreScreen::PurchaseItem(StoreItem theStoreItem)
{
    ...
    {
        Dialog* aDialog = mApp->DoDialog(DIALOG_NOT_ENOUGH_MONEY, true, "Not enough money"/*[NOT_ENOUGH_MONEY]*/, "You can't afford this item yet. Earn more coins by killing zombies!"/*[CANNOT_AFFORD_ITEM]*/, "[DIALOG_BUTTON_OK]", BUTTONS_FOOTER);
        mWaitForDialog = true;
    ...
    else
    {
        ...
        if (aComfirmResult == ID_OK)
        {
            ...
            if (theStoreItem == STORE_ITEM_PACKET_UPGRADE)
            {
                ...
                PopString aDialogLines = StrFormat("Now you can choose to take %d seeds with you per level!", 6 + mApp->mPlayerInfo->mPurchases[theStoreItem]);
                Dialog* aDialog = mApp->DoDialog(DIALOG_UPGRADED, true, "More slots!"/*[MORE_SLOTS]*/, aDialogLines, "[DIALOG_BUTTON_OK]", BUTTONS_FOOTER);
...
void StoreScreen::AdvanceCrazyDaveDialog()
{
    ...
    if (mApp->mCrazyDaveMessageIndex == 3100)
    {
        mHatchTimer = 150;
        mHatchOpen = true;
        mApp->PlaySample(PopLib::SOUND_HATCHBACK_OPEN);
    }
    ...
    else if (aMessage == 603)
    {
        ...
        mApp->WriteCurrentUserConfig();
        mApp->PlaySample(PopLib::SOUND_DIAMOND);
