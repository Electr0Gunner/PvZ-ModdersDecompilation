...
//0x45D9D0
void NewUserDialog::EditWidgetText(int theId, const PopString& theString)
{
	mApp->ButtonDepress(mId + 2000);
}

//0x45D9F0
bool NewUserDialog::AllowChar(int, PopChar theChar)
{
	return isalnum(theChar) || theChar == ' ';
}

//0x45DA20
PopString NewUserDialog::GetName()
{
	PopString aString;
	PopChar aLastChar = ' ';
	...
	{
		PopChar aChar = mNameEditWidget->mString[i];
		if (aChar != ' ')
		{
...
void NewUserDialog::SetName(const PopString& theName)
{
