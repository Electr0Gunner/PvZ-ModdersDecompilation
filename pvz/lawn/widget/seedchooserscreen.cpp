...
#include "lawn/widget/imitaterdialog.hpp"
...
SeedChooserScreen::SeedChooserScreen()
{
	...
	mStartButton->SetLabel("[LETS_ROCK_BUTTON]");
	...
	mRandomButton->mButtonImage = PopLib::IMAGE_BLANK;
	mRandomButton->mOverImage = PopLib::IMAGE_BLANK;
	mRandomButton->mDownImage = PopLib::IMAGE_BLANK;
	mRandomButton->SetFont(PopLib::FONT_BRIANNETOD12);
	...
	Image* aBtnImage = PopLib::IMAGE_SEEDCHOOSER_BUTTON2;
	Image* aOverImage = PopLib::IMAGE_SEEDCHOOSER_BUTTON2_GLOW;
	...
	mViewLawnButton->SetFont(PopLib::FONT_BRIANNETOD12);
	...
	mAlmanacButton->SetFont(PopLib::FONT_BRIANNETOD12);
	...
	mStoreButton->SetFont(PopLib::FONT_BRIANNETOD12);
	...
	mImitaterButton->mButtonImage = PopLib::IMAGE_IMITATERSEED;
	mImitaterButton->mOverImage = PopLib::IMAGE_IMITATERSEED;
	mImitaterButton->mDownImage = PopLib::IMAGE_IMITATERSEED;
	mImitaterButton->mDisabledImage = PopLib::IMAGE_IMITATERSEEDDISABLED;
	mImitaterButton->Resize(464, 515, PopLib::IMAGE_IMITATERSEED->mWidth, PopLib::IMAGE_IMITATERSEED->mHeight);
...
void SeedChooserScreen::Draw(Graphics* g)
{
	...
	g->DrawImage(PopLib::IMAGE_SEEDCHOOSER_BACKGROUND, 0, 87);
	if (mApp->SeedTypeAvailable(SEED_IMITATER))
	{
		g->DrawImage(PopLib::IMAGE_SEEDCHOOSER_IMITATERADDON, 459, 503);
	}
	TodDrawString(g, "[CHOOSE_YOUR_PLANTS]", 229, 110, PopLib::FONT_DWARVENTODCRAFT18YELLOW, Color::White, DS_ALIGN_CENTER);
	...
	for (SeedType aSeedShadow = SEED_PEASHOOTER; aSeedShadow < aNumSeeds; aSeedShadow = (SeedType)(aSeedShadow + 1))
	{
		...
		{
			g->DrawImage(PopLib::IMAGE_SEEDPACKETSILHOUETTE, x, y);
		}
	}
...
void SeedChooserScreen::UpdateViewLawn()
{
	...
	int aSeedChooserY = SEED_CHOOSER_OFFSET_Y - PopLib::IMAGE_SEEDCHOOSER_BACKGROUND->mHeight;
...
//0x485340
bool SeedChooserScreen::DisplayRepickWarningDialog(const PopChar* theMessage)
{
...
bool SeedChooserScreen::CheckSeedUpgrade(SeedType theSeedTypeTo, SeedType theSeedTypeFrom)
{
	...
	PopString aWarning = TodStringTranslate("[SEED_CHOOSER_UPGRADE_WARNING]");
...
//0x486EA0
void SeedChooserScreen::KeyChar(PopChar theChar)
{
