...
#include "lawn/widget/gameselector.hpp"
#include "lawnapp.hpp"
...
GameSelector::GameSelector(LawnApp* theApp)
{
	...
	mAdventureButton->Resize(0, 0, PopLib::IMAGE_REANIM_SELECTORSCREEN_ADVENTURE_BUTTON->mWidth, 125);
	mAdventureButton->mClip = false;
	mAdventureButton->mBtnNoDraw = true;
	mAdventureButton->mMouseVisible = false;
	mAdventureButton->mPolygonShape[0] = Vector2(7.0f, 1.0f);
	mAdventureButton->mPolygonShape[1] = Vector2(328.0f, 30.0f);
	mAdventureButton->mPolygonShape[2] = Vector2(314.0f, 125.0f);
	mAdventureButton->mPolygonShape[3] = Vector2(1.0f, 78.0f);
	...
	mMinigameButton->Resize(0, 0, PopLib::IMAGE_REANIM_SELECTORSCREEN_SURVIVAL_BUTTON->mWidth, 130);
	mMinigameButton->mClip = false;
	mMinigameButton->mBtnNoDraw = true;
	mMinigameButton->mMouseVisible = false;
	mMinigameButton->mPolygonShape[0] = Vector2(4.0f, 2.0f);
	mMinigameButton->mPolygonShape[1] = Vector2(312.0f, 51.0f);
	mMinigameButton->mPolygonShape[2] = Vector2(296.0f, 130.0f);
	mMinigameButton->mPolygonShape[3] = Vector2(7.0f, 77.0f);
	...
	mPuzzleButton->Resize(0, 0, PopLib::IMAGE_REANIM_SELECTORSCREEN_CHALLENGES_BUTTON->mWidth, 121);
	mPuzzleButton->mClip = false;
	mPuzzleButton->mBtnNoDraw = true;
	mPuzzleButton->mMouseVisible = false;
	mPuzzleButton->mPolygonShape[0] = Vector2(2.0f, 0.0f);
	mPuzzleButton->mPolygonShape[1] = Vector2(281.0f, 55.0f);
	mPuzzleButton->mPolygonShape[2] = Vector2(268.0f, 121.0f);
	mPuzzleButton->mPolygonShape[3] = Vector2(3.0f, 60.0f);
	...
	mSurvivalButton->Resize(0, 0, PopLib::IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_BUTTON->mWidth, 124);
	mSurvivalButton->mClip = false;
	mSurvivalButton->mBtnNoDraw = true;
	mSurvivalButton->mMouseVisible = false;
	mSurvivalButton->mPolygonShape[0] = Vector2(7.0f, 1.0f);
	mSurvivalButton->mPolygonShape[1] = Vector2(267.0f, 62.0f);
	mSurvivalButton->mPolygonShape[2] = Vector2(257.0f, 124.0f);
	mSurvivalButton->mPolygonShape[3] = Vector2(7.0f, 57.0f);
	...
	mOptionsButton->Resize(0, 0, PopLib::IMAGE_SELECTORSCREEN_OPTIONS1->mWidth, PopLib::IMAGE_SELECTORSCREEN_OPTIONS1->mHeight + 23);
	...
	mHelpButton->Resize(0, 0, PopLib::IMAGE_SELECTORSCREEN_HELP1->mWidth, PopLib::IMAGE_SELECTORSCREEN_HELP1->mHeight + 33);
	...
	mQuitButton->Resize(0, 0, PopLib::IMAGE_SELECTORSCREEN_QUIT1->mWidth + 10, PopLib::IMAGE_SELECTORSCREEN_QUIT1->mHeight + 10);
	...
	mStoreButton->Resize(405, 484, PopLib::IMAGE_SELECTORSCREEN_STORE->mWidth, PopLib::IMAGE_SELECTORSCREEN_STORE->mHeight);
	...
	mAlmanacButton->Resize(327, 428, PopLib::IMAGE_SELECTORSCREEN_ALMANAC->mWidth, PopLib::IMAGE_SELECTORSCREEN_ALMANAC->mHeight);
	...
	for (int i = 0; i < 6; i++)
	{
		...
		std::string aAnimName = PopLib::StrFormat("anim_cloud%d", (i > 1 ? i + 2 : i + 1));
	...
	for (int i = 0; i < 3; i++)
	{
		...
		std::string aAnimName = PopLib::StrFormat("anim_flower%d", i + 1);
	...
	SyncProfile(false);
	mApp->PlaySample(PopLib::SOUND_ROLL_IN);
...
void GameSelector::SyncButtons()
{
	...
	if (aAlmanacAvailable)
	{
		...
		if (aStoreOpen)
			...
			aSelectorReanim->SetImageOverride("almanac_key_shadow", PopLib::IMAGE_REANIM_SELECTORSCREEN_ALMANAC_SHADOW);
	}
	else if (aStoreOpen)
	{
		aSelectorReanim->AssignRenderGroupToPrefix("almanac_key_shadow", RENDER_GROUP_NORMAL);
		aSelectorReanim->SetImageOverride("almanac_key_shadow", PopLib::IMAGE_REANIM_SELECTORSCREEN_KEY_SHADOW);
	}
	else
	...
	if (mMinigamesLocked)
	{
		mMinigameButton->mOverImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_SURVIVAL_BUTTON;
		mMinigameButton->mDownImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_SURVIVAL_BUTTON;
		mMinigameButton->SetColor(ButtonWidget::COLOR_BKG, Color(128, 128, 128));
	}
	else
	{
		mMinigameButton->mOverImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_SURVIVAL_HIGHLIGHT;
		mMinigameButton->mDownImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_SURVIVAL_HIGHLIGHT;
		mMinigameButton->SetColor(ButtonWidget::COLOR_BKG, Color::White);
	}
	if (mPuzzleLocked)
	{
		mPuzzleButton->mOverImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_CHALLENGES_BUTTON;
		mPuzzleButton->mDownImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_CHALLENGES_BUTTON;
		mPuzzleButton->SetColor(ButtonWidget::COLOR_BKG, Color(128, 128, 128));
	}
	else
	{
		mPuzzleButton->mOverImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_CHALLENGES_HIGHLIGHT;
		mPuzzleButton->mDownImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_CHALLENGES_HIGHLIGHT;
		mPuzzleButton->SetColor(ButtonWidget::COLOR_BKG, Color::White);
	}
	if (mSurvivalLocked)
	{
		mSurvivalButton->mOverImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_BUTTON;
		mSurvivalButton->mDownImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_BUTTON;
		mSurvivalButton->SetColor(ButtonWidget::COLOR_BKG, Color(128, 128, 128));
	}
	else
	{
		mSurvivalButton->mOverImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_HIGHLIGHT;
		mSurvivalButton->mDownImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_HIGHLIGHT;
	...
	if (mShowStartButton)
	{
		...
		mAdventureButton->mButtonImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_STARTADVENTURE_BUTTON;
		mAdventureButton->mOverImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_STARTADVENTURE_HIGHLIGHT;
		mAdventureButton->mDownImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_STARTADVENTURE_HIGHLIGHT;
	}
	else
	{
		aSelectorReanim->AssignRenderGroupToPrefix("SelectorScreen_StartAdventure_", RENDER_GROUP_HIDDEN);
		mAdventureButton->mButtonImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_ADVENTURE_BUTTON;
		mAdventureButton->mOverImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_ADVENTURE_HIGHLIGHT;
		mAdventureButton->mDownImage = PopLib::IMAGE_REANIM_SELECTORSCREEN_ADVENTURE_HIGHLIGHT;
	}
}
...
void GameSelector::Draw(Graphics* g)
{
	...
	{
		PopString aWelcomeStr = mApp->mPlayerInfo->mName + '!';

		int aSignIdx = aSelectorReanim->FindTrackIndex("woodsign1");
		Transform2D aOverlayMatrix;
		aSelectorReanim->GetAttachmentOverlayMatrix(aSignIdx, aOverlayMatrix);
		float aStringWidth = PopLib::FONT_BRIANNETOD16->StringWidth(aWelcomeStr);
		Transform2D aOffsetMatrix;
		aOffsetMatrix.Translate(170.5f - (int)(aStringWidth * 0.5f), 102.5f);
		TodDrawStringMatrix(g, PopLib::FONT_BRIANNETOD16, aOverlayMatrix * aOffsetMatrix, aWelcomeStr, Color(255, 245, 200));
	...
	if (mHasTrophy)
	{
		if (mApp->EarnedGoldTrophy())
			TodDrawImageCelF(g, PopLib::IMAGE_SUNFLOWER_TROPHY, aTransformLeft.mTransX + 10.0f, aTransformLeft.mTransY + 390.0f, 1, 0);
		else
			TodDrawImageCelF(g, PopLib::IMAGE_SUNFLOWER_TROPHY, aTransformLeft.mTransX + 10.0f, aTransformLeft.mTransY + 390.0f, 0, 0);
...
void GameSelector::DrawOverlay(Graphics* g)
{
	...
	if (!mApp->IsIceDemo() && !mShowStartButton)
	{
		...
		TodDrawImageCelF(g, PopLib::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransAreaX + 486.0f, aTransAreaY + 125.0f, aStage, 0);  // ���ƴ����
		if (aSub < 10)
		{
			TodDrawImageCelF(g, PopLib::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 504.0f, aTransSubY + 128.0f, aSub, 0);
		}
		else if (aSub == 10)
		{
			TodDrawImageCelF(g, PopLib::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 504.0f, aTransSubY + 128.0f, 1, 0);
			TodDrawImageCelF(g, PopLib::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 513.0f, aTransSubY + 129.0f, 0, 0);
		}
	...
	{
		g->DrawImage(PopLib::IMAGE_PLANTSPEECHBUBBLE, mZenGardenButton->mX + 106, mZenGardenButton->mY + 36);
		g->DrawImage(PopLib::IMAGE_WATERDROP, mZenGardenButton->mX + 123, mZenGardenButton->mY + 45);
	}
	...
	{
		g->SetFont(PopLib::FONT_BRIANNETOD16);
...
void GameSelector::Update()
{
	...
	if (mStartingGame)
	{
		...
		if (mStartingGameCounter == 125)
			mApp->PlaySample(PopLib::SOUND_EVILLAUGH);
	}
	...
	aSelectorReanim->SetImageOverride("woodsign2", (mChangeUserButton->mIsOver || mChangeUserButton->mIsDown) ? PopLib::IMAGE_REANIM_SELECTORSCREEN_WOODSIGN2_PRESS : nullptr);
}
...
void GameSelector::KeyDown(KeyCode theKey)
{
	...
	if (mApp->mDaisyCheck->Check(theKey))
	{
		...
		{
			mApp->PlaySample(PopLib::SOUND_LOADINGBAR_FLOWER);
