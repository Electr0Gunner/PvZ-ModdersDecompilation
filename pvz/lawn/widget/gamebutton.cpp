...
//0x447B00
void DrawStoneButton(Graphics* g, int x, int y, int theWidth, int theHeight, bool isDown, bool isHighLighted, const PopString& theLabel)
{
	Image* aLeftImage = PopLib::IMAGE_BUTTON_LEFT;
	Image* aMiddleImage = PopLib::IMAGE_BUTTON_MIDDLE;
	Image* aRightImage = PopLib::IMAGE_BUTTON_RIGHT;
	int aFontX = x;
	int aFontY = y;
	int aImageX = x;
	if (isDown)
	{
		aLeftImage = PopLib::IMAGE_BUTTON_DOWN_LEFT;
		aMiddleImage = PopLib::IMAGE_BUTTON_DOWN_MIDDLE;
		aRightImage = PopLib::IMAGE_BUTTON_DOWN_RIGHT;
		aFontX++;
		aFontY++;
		aImageX++;
	}
	...
	g->SetFont(isHighLighted ? PopLib::FONT_DWARVENTODCRAFT18BRIGHTGREENINSET : PopLib::FONT_DWARVENTODCRAFT18GREENINSET);
	aFontX += (theWidth - PopLib::FONT_DWARVENTODCRAFT18GREENINSET->StringWidth(theLabel)) / 2 + 1;
	aFontY += (theHeight - PopLib::FONT_DWARVENTODCRAFT18GREENINSET->GetAscent() / 6 - 1 + PopLib::FONT_DWARVENTODCRAFT18GREENINSET->GetAscent()) / 2 - 4;
...
void GameButton::Draw(Graphics* g)
{
	...
	if (!isDown)
	{
		...
		{
			if (HaveButtonImage(mButtonImage, mNormalRect) && mOverAlpha < 1.0f)  // δ��ȫ���ɽ���
...
//0x448470
void GameButton::SetLabel(const PopString& theLabel)
{
	mLabel = TodStringTranslate(theLabel);
}

//0x4484E0
void NewLawnButton::SetLabel(const PopString& theLabel)
{
	mLabel = TodStringTranslate(theLabel);
}

//0x448550
void LawnStoneButton::SetLabel(const PopString& theLabel)
{
	mLabel = TodStringTranslate(theLabel);
}
...
//0x448620
LawnStoneButton* MakeButton(int theId, ButtonListener* theListener, const PopString& theText)
{
...
bool NewLawnButton::IsPointVisible(int x, int y)
{
	...
	return TodIsPointInPolygon(mPolygonShape, 4, Vector2(x, y));
}

//0x448BC0
NewLawnButton* MakeNewButton(int theId, ButtonListener* theListener, const PopString& theText, Font* theFont, Image* theImageNormal, Image* theImageOver, Image* theImageDown)
{
	NewLawnButton* aButton = new NewLawnButton(nullptr, theId, theListener);
	aButton->SetFont(theFont == nullptr ? PopLib::FONT_BRIANNETOD12 : theFont);
	aButton->SetLabel(theText);
