...
AlmanacDialog::AlmanacDialog(LawnApp* theApp) : LawnDialog(theApp, DIALOG_ALMANAC, true, "Almanac", "", "", BUTTONS_NONE)
{
	...
	mCloseButton->mButtonImage = PopLib::IMAGE_ALMANAC_CLOSEBUTTON;
	mCloseButton->mOverImage = PopLib::IMAGE_ALMANAC_CLOSEBUTTONHIGHLIGHT;
	mCloseButton->mDownImage = nullptr;
	mCloseButton->SetFont(PopLib::FONT_BRIANNETOD12);
	...
	mIndexButton->mButtonImage = PopLib::IMAGE_ALMANAC_INDEXBUTTON;
	mIndexButton->mOverImage = PopLib::IMAGE_ALMANAC_INDEXBUTTONHIGHLIGHT;
	mIndexButton->mDownImage = nullptr;
	mIndexButton->SetFont(PopLib::FONT_BRIANNETOD12);
	...
	mPlantButton->mButtonImage = PopLib::IMAGE_SEEDCHOOSER_BUTTON;
	mPlantButton->mOverImage = nullptr;
	mPlantButton->mDownImage = nullptr;
	mPlantButton->mDisabledImage = PopLib::IMAGE_SEEDCHOOSER_BUTTON_DISABLED;
	mPlantButton->mOverOverlayImage = PopLib::IMAGE_SEEDCHOOSER_BUTTON_GLOW;
	mPlantButton->SetFont(PopLib::FONT_DWARVENTODCRAFT18YELLOW);
...
{
	g->DrawImage(PopLib::IMAGE_ALMANAC_INDEXBACK, 0, 0);
	TodDrawString(g, "[SUBURBAN_ALMANAC_INDEX]", BOARD_WIDTH / 2, 60, PopLib::FONT_HOUSEOFTERROR28, Color(220, 220, 220), DrawStringJustification::DS_ALIGN_CENTER);
...
{
	g->DrawImage(PopLib::IMAGE_ALMANAC_PLANTBACK, 0, 0);
	TodDrawString(g, "[SUBURBAN_ALMANAC_PLANTS]", BOARD_WIDTH / 2, 48, PopLib::FONT_HOUSEOFTERROR20, Color(213, 159, 43), DrawStringJustification::DS_ALIGN_CENTER);
		...
		if (mApp->SeedTypeAvailable(aSeedType))
		{
			if (aSeedType == SeedType::SEED_IMITATER)
			{
				if (aSeedType == aSeedMouseOn)
					g->DrawImage(PopLib::IMAGE_ALMANAC_IMITATER, aPosX, aPosY);
				g->DrawImage(PopLib::IMAGE_ALMANAC_IMITATER, aPosX, aPosY);
			}
			else
			{
		...
		mSelectedSeed == SeedType::SEED_CATTAIL || mSelectedSeed == SeedType::SEED_SEASHROOM)
	{
		...
		g->DrawImage(aNight ? PopLib::IMAGE_ALMANAC_GROUNDNIGHTPOOL : PopLib::IMAGE_ALMANAC_GROUNDPOOL, 521, 107);
	...
	{
		g->DrawImage(
			Plant::IsNocturnal(mSelectedSeed) || mSelectedSeed == SeedType::SEED_GRAVEBUSTER || mSelectedSeed == SeedType::SEED_PLANTERN ? PopLib::IMAGE_ALMANAC_GROUNDNIGHT :
			mSelectedSeed == SeedType::SEED_FLOWERPOT ? PopLib::IMAGE_ALMANAC_GROUNDROOF : PopLib::IMAGE_ALMANAC_GROUNDDAY,
			521, 107
		);
	}
	...
	g->DrawImage(PopLib::IMAGE_ALMANAC_PLANTCARD, 459, 86);
	PlantDefinition& aPlantDef = GetPlantDefinition(mSelectedSeed);
	PopString aName = Plant::GetNameString(mSelectedSeed, SEED_NONE);
	PopString aDescriptionName = StrFormat("[%s_DESCRIPTION]", aPlantDef.mPlantName);
	TodDrawString(g, aName, 617, 288, PopLib::FONT_DWARVENTODCRAFT18YELLOW, Color::White, DS_ALIGN_CENTER);
	TodDrawStringWrapped(g, aDescriptionName, Rect(485, 309, 258, 230), PopLib::FONT_BRIANNETOD12, Color(40, 50, 90), DS_ALIGN_LEFT);

	if (mSelectedSeed != SeedType::SEED_IMITATER)
	{
		PopString aCostStr = TodReplaceString(StrFormat("{KEYWORD}{COST}:{STAT} %d", aPlantDef.mSeedCost), "{COST}", "[COST]");
		TodDrawStringWrapped(g, aCostStr, Rect(485, 520, 134, 50), PopLib::FONT_BRIANNETOD12, Color::White, DS_ALIGN_LEFT);

		PopString aRechargeStr = TodReplaceString(
		...
		TodDrawStringWrapped(g, aRechargeStr, Rect(600, 520, 139, 50), PopLib::FONT_BRIANNETOD12, Color(40, 50, 90), DS_ALIGN_RIGHT);
	}
}
...
{
	g->DrawImage(PopLib::IMAGE_ALMANAC_ZOMBIEBACK, 0, 0);
	TodDrawString(g, "[SUBURBAN_ALMANAC_ZOMBIES]", BOARD_WIDTH / 2, 54, PopLib::FONT_DWARVENTODCRAFT24, Color(0, 196, 0), DS_ALIGN_CENTER);
	...
	for (int i = 0; i < NUM_ALMANAC_ZOMBIES; i++)
	{
		...
		if (aZombieType != ZombieType::ZOMBIE_INVALID)
		{
			if (!ZombieIsShown(aZombieType))
				g->DrawImage(PopLib::IMAGE_ALMANAC_ZOMBIEBLANK, aPosX, aPosY);
			else
			{
				g->DrawImage(PopLib::IMAGE_ALMANAC_ZOMBIEWINDOW, aPosX, aPosY);
				...
				g->DrawImage(PopLib::IMAGE_ALMANAC_ZOMBIEWINDOW2, aPosX, aPosY);
				if (aZombieType == aZombieMouseOn)
				{
					g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);
					g->SetColor(Color(255, 255, 255, 48));
	...
	g->DrawImage(mZombie->mZombieType == ZombieType::ZOMBIE_ZAMBONI || mZombie->mZombieType == ZombieType::ZOMBIE_BOBSLED ?
		PopLib::IMAGE_ALMANAC_GROUNDICE : PopLib::IMAGE_ALMANAC_GROUNDDAY, 518, 110);
	...
	g->DrawImage(PopLib::IMAGE_ALMANAC_ZOMBIECARD, 455, 78);

	ZombieDefinition& aZombieDef = GetZombieDefinition(mSelectedZombie);
	PopString aName = ZombieHasSilhouette(mSelectedZombie) ? "???" : StrFormat("[%s]", aZombieDef.mZombieName);
	TodDrawString(g, aName, 613, 362, PopLib::FONT_DWARVENTODCRAFT18GREENINSET, Color(190, 255, 235, 255), DS_ALIGN_CENTER);

	PopString aDescription;
	...
	TodDrawStringWrapped(g, aDescription, Rect(484, 377, 258, 170), PopLib::FONT_BRIANNETOD12, Color(40, 50, 90), aAlign);
}
