...
//0x456A80
LawnDialog::LawnDialog(LawnApp* theApp, int theId, bool isModal, const PopString& theDialogHeader, const PopString& theDialogLines, const PopString& theDialogFooter, int theButtonMode) :
	Dialog(nullptr, nullptr, theId, isModal, theDialogHeader, theDialogLines, "", BUTTONS_NONE)
{
    ...
    SetHeaderFont(PopLib::FONT_DWARVENTODCRAFT24);
    SetLinesFont(PopLib::FONT_DWARVENTODCRAFT15);
...
void LawnDialog::CalcSize(int theExtraX, int theExtraY)
{
    ...
    // 对话框贴图的最小宽度
    int aTopMidWidth = PopLib::IMAGE_DIALOG_TOPMIDDLE->mWidth;
    int aImageWidth = PopLib::IMAGE_DIALOG_TOPLEFT->mWidth + PopLib::IMAGE_DIALOG_TOPRIGHT->mWidth + aTopMidWidth;
    // 计算最终的宽度
    ...
    // 对话框贴图的最小高度
    int aBottomHeight = (mTallBottom ? PopLib::IMAGE_DIALOG_BIGBOTTOMLEFT : PopLib::IMAGE_DIALOG_BOTTOMLEFT)->mHeight;
    int aImageHeight = PopLib::IMAGE_DIALOG_TOPLEFT->mHeight + aBottomHeight + DIALOG_HEADER_OFFSET;
    // 计算最终的高度
    ...
    {
        int aCenterHeight = PopLib::IMAGE_DIALOG_CENTERLEFT->mHeight;
...
//0x457BC0
GameOverDialog::GameOverDialog(const PopString& theMessage, bool theShowChallengeName) : LawnDialog(
    gLawnApp, 
