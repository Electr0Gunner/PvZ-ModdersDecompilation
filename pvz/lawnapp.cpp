...
#include "lawn/widget/userdialog.hpp"
...
#include "lawn/widget/cheatdialog.hpp"
...
#include "Lawn/widget/continuedialog.hpp"
...
LawnApp::LawnApp()
{
	...
	//mBigArrowCursor = LoadCursor(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDC_CURSOR1)); TODO: ADD
	mDRM = nullptr;
}
...
//0x4504B0
int LawnApp::LawnMessageBox(int theDialogId, const PopChar* theHeaderName, const PopChar* theLinesName, const PopChar* theButton1Name, const PopChar* theButton2Name, int theButtonMode)
{
...
//0x450770
Dialog* LawnApp::DoDialog(int theDialogId, bool isModal, const PopString& theDialogHeader, const PopString& theDialogLines, const PopString& theDialogFooter, int theButtonMode)
{
	PopString aHeader = TodStringTranslate(theDialogHeader);
	PopString aLines = TodStringTranslate(theDialogLines);
	PopString aFooter = TodStringTranslate(theDialogFooter);

	Dialog* aDialog = AppBase::DoDialog(theDialogId, isModal, aHeader, aLines, aFooter, theButtonMode);
...
Dialog* LawnApp::DoDialogDelay(int theDialogId, bool isModal, const PopString& theDialogHeader, const PopString& theDialogLines, const PopString& theDialogFooter, int theButtonMode)
{
	LawnDialog* aDialog = (LawnDialog*)AppBase::DoDialog(theDialogId, isModal, theDialogHeader, theDialogLines, theDialogFooter, theButtonMode);
...
void LawnApp::FinishCreateUserDialog(bool isYes)
{
	...
	PopString aName = aNewUserDialog->GetName();
...
//0x450E20
void LawnApp::DoConfirmDeleteUserDialog(const PopString& theName)
{
	...
	DoDialog(
		...
		// StrFormat(TodStringTranslate(_S("[DELETE_USER_WARNING]")).c_str(), StringToPopStringFast(theName))
...
void LawnApp::FinishConfirmDeleteUserDialog(bool isYes)
{
	...
	PopString aCurName = mPlayerInfo ? mPlayerInfo->mName : "";
	PopString aName = aUserDialog->GetSelName();
	if (aName == aCurName)
	{
		mPlayerInfo = nullptr;
	}
...
//0x451180
void LawnApp::DoRenameUserDialog(const PopString& theName)
{
...
void LawnApp::FinishRenameUserDialog(bool isYes)
{
	...
	PopString anOldName = aUserDialog->GetSelName();
	PopString aNewName = aNewUserDialog->GetName();
	if (aNewName.empty())
		return;
...
void LawnApp::DoConfirmSellDialog(const PopString& theMessage)
{
...
void LawnApp::DoConfirmPurchaseDialog(const PopString& theMessage)
{
...
//0x451580
Dialog* LawnApp::NewDialog(int theDialogId, bool isModal, const PopString& theDialogHeader, const PopString& theDialogLines, const PopString& theDialogFooter, int theButtonMode)
{
...
{
	if (AppBase::KillDialog(theDialogId))
	{
...
{
	AppBase::ShowResourceError(doExit);
}
...
{
	return AppBase::DebugKeyDown(theKey);
}
...
void LawnApp::HandleCmdLineParam(const std::string& theParamName, const std::string& theParamValue)
{
	...
	{
		PopApp::HandleCmdLineParam(theParamName, theParamValue);
	}
}
...
void LawnApp::LoadGroup(const char* theGroupName, int theGroupAveMsToLoad)
{
	...
	int aGroupTime = std::fmax(aTimer.GetDuration(), 0);
...
void LawnApp::LoadingThreadProc()
{
	...
	mMusic->MusicInit();
	int aDuration = std::fmax(aTimer.GetDuration(), 0);
	aTimer.Start();
	...
	aDuration = std::fmax(aTimer.GetDuration(), 0);
	aTimer.Start();
...
bool LawnApp::OpenURL(const std::string& theURL, bool shutdownOnOpen)
{
	...
	return AppBase::OpenURL(theURL, shutdownOnOpen);
}

//0x453040
void LawnApp::ConfirmQuit()
{
	PopString aBody = TodStringTranslate("[QUIT_MESSAGE]");
	PopString aHeader = TodStringTranslate("[QUIT_HEADER]");
...
//0x453670
PopString LawnApp::GetStageString(int theLevel)
{
...
bool LawnApp::UpdateApp()
{
	...
	bool updated = AppBase::UpdateApp();
...
{
	PopString aMessageName = StrFormat("[CRAZY_DAVE_%d]", mCrazyDaveMessageIndex + 1);
...
//0x453DC0
PopString LawnApp::GetCrazyDaveText(int theMessageIndex)
{
	PopString aMessage = StrFormat("[CRAZY_DAVE_%d]", theMessageIndex);
...
void LawnApp::CrazyDaveTalkIndex(int theMessageIndex)
{
	...
	PopString aMessageText = GetCrazyDaveText(theMessageIndex);
...
//0x454570
void LawnApp::CrazyDaveTalkMessage(const PopString& theMessage)
{
	Reanimation* aCrazyDaveReanim = ReanimationGet(mCrazyDaveReanimID);

	bool doHanding = false;
	if (theMessage.find("{HANDING}") != PopString::npos)
	{
		doHanding = true;
	}
	...
	bool doSound = true;
	if (theMessage.find("{NO_SOUND}") != PopString::npos)
	{
		doSound = false;
	}
	else
	...
	if (mCrazyDaveState != CrazyDaveState::CRAZY_DAVE_TALKING || doSound)
	{
		if (doHanding)
		{
			...
			if (doSound)
			{
				if (theMessage.find("{SHORT_SOUND}") != PopString::npos)
				{
					PlayFoley(FoleyType::FOLEY_CRAZY_DAVE_SHORT);
				}
				else if (theMessage.find("{SCREAM}") != PopString::npos)
				{
		...
		else if (theMessage.find("{SHAKE}") != PopString::npos)
		{
		...
		else if (theMessage.find("{SCREAM2}") != PopString::npos)
		{
		...
		else if (theMessage.find("{SHOW_WALLNUT}") != PopString::npos)
		{
			aCrazyDaveReanim->PlayReanim("anim_talk_handing", ReanimLoopType::REANIM_LOOP, 50, 12.0f);
		...
		else if (theMessage.find("{SHOW_HAMMER}") != PopString::npos)
		{
			aCrazyDaveReanim->PlayReanim("anim_talk_handing", ReanimLoopType::REANIM_LOOP, 50, 12.0f);
		...
		else if (theMessage.find("{SHOW_FERTILIZER}") != PopString::npos)
		{
			aCrazyDaveReanim->PlayReanim("anim_talk_handing", ReanimLoopType::REANIM_LOOP, 50, 12.0f);
		...
		else if (theMessage.find("{SHOW_TREE_FOOD}") != PopString::npos)
		{
			aCrazyDaveReanim->PlayReanim("anim_talk_handing", ReanimLoopType::REANIM_LOOP, 50, 12.0f);
		...
		else if (theMessage.find("{SHOW_MONEYBAG}") != PopString::npos)
		{
			aCrazyDaveReanim->PlayReanim("anim_talk_handing", ReanimLoopType::REANIM_LOOP, 50, 12.0f);
...
void LawnApp::DrawCrazyDave(Graphics* g)
{
	...
	if (mCrazyDaveMessageText.size())
	{
		...
		PopString aBubbleText = mCrazyDaveMessageText;
		Rect aRect(aPosX + 25, aPosY + 6, 233, 144);
		if (aBubbleText.find("{SHAKE}") != PopString::npos)
		{
		...
		else if (aBubbleText.find("{NO_CLICK}") != PopString::npos)
		{
...
//0x455AA0
PopString LawnApp::Pluralize(int theCount, const PopChar* theSingular, const PopChar* thePlural)
{
	if (theCount == 1)
	{
...
//0x455E10
PopString LawnApp::GetMoneyString(int theAmount)
{
...
//0x455EE0
PopString LawnGetCurrentLevelName()
{
	if (gLawnApp == nullptr)
	{
		return "Before App";
...
{
	AppBase::SwitchScreenMode(wantWindowed, is3d, force);
